...
**Status:** living schema doctrine; PR13-PR54 are merged or in review, PR54 completed reviewed source-backed coverage for all 222 runtime models, and the current posture is `full_reviewed_source_backed_coverage_complete`
...
- Batch 10 records: `data/model_affordances/batch_10/` for 12 controlled
  frame correction, metacognition, counterfactual, evidence-boundary, and
  false-precision extractions opened after PR44 graph-only priority audit.
...
- Compiled v13 artifact:
  `data/compiled/model_affordances/affordances_v13.json`.
- v13 corpus shape: `158` reviewed records, `194` affordances, `301` absence
  records. v13 remains `draft_review_only` and is not runtime-promoted.
- Compiled v14 artifact:
  `data/compiled/model_affordances/affordances_v14.json`.
- v14 corpus shape: `170` reviewed records, `206` affordances, `325` absence
  records. v14 remains `draft_review_only` and is not runtime-promoted.
- Batch 14 records: `data/model_affordances/batch_14/` for 12 controlled
  self-regulation, bias-calibration, motivation, agency, and regret
  extractions opened after PR50.
- Compiled v15 artifact:
...
- Batch 17 records: `data/model_affordances/batch_17/` for the final 16
  graph-only runtime models: agile methods, causal attribution resistance,
  chain of thought, competence boundaries, complexity resistance, endowment,
  latticework, fallacy checks, reality maps, metacognition, perceptual
  learning, scaffolding, System 1, System 2, Tier-2 High Value, and
  time-tested validation.
- Compiled v18 artifact:
  `data/compiled/model_affordances/affordances_v18.json`.
- v18 corpus shape: `222` reviewed records, `258` affordances, `429` absence
  records. v18 remains `draft_review_only` and is not runtime-promoted.
...
- PR44 completed one after-v10 graph-only priority audit after PR43. It reviews
  the remaining `100` graph-only runtime models, compares candidate families,
  and recommends frame correction / metacognitive blind-spot discipline as the
  next controlled enrichment family because future packets are likely to be
  thin where otherwise operational advice is being evaluated through the wrong
  frame, reasoning mode, evidence boundary, or counterfactual. Decision label:
  `v10_graph_only_priority_audit_complete`.
- PR45 completed one controlled frame-correction / metacognitive enrichment
  batch after PR44. It adds `12` Batch 10 records for named frame correction,
  metacognition, counterfactual reasoning, evidence-boundary, and
...
- PR47 completed one after-v11 graph-only priority audit after PR46. It reviews
  the remaining `88` graph-only runtime models and recommends adaptive
  exploration / option generation / synthesis discipline as the next
  controlled enrichment family because future packets may still be trapped
...
- PR49 completed one controlled learning / skill-acquisition enrichment batch
  after PR48. It adds `12` Batch 12 records for mastery sequencing, cognitive
  load, deliberate practice, desirable difficulty, expertise matching,
  Feynman explanation, generation, learning curves, scaffolding, schema
  acquisition, varied practice, and reachable stretch. It compiles
  draft/review-only v13 and preserves `24` absence records. Decision label:
  `controlled_learning_skill_enrichment_ready`.
- PR50 completed one controlled quantitative inference / distributional
  reasoning enrichment batch after PR49. It adds `12` Batch 13 records for
  Bayesian updating, regression-to-mean baselines, conjunction-risk checks,
  representativeness, Monte Carlo ranges, Markov transition caveats,
  statistics, statistical learning, data-science framing, information
  compression, power laws, and compounding. It compiles draft/review-only v14
  and preserves `24` absence records. Decision label:
  `controlled_quantitative_inference_enrichment_ready`.
- PR51 completed one controlled self-regulation / bias-calibration enrichment
  batch after PR50. It adds `12` Batch 14 records for cognitive biases,
  cognitive dissonance, rationalization, Dunning-Kruger calibration, curse of
  knowledge, hindsight bias, locus of control, self-control, self-determination,
...
- PR54 completed the final controlled graph-only enrichment batch after PR53.
  It adds `16` Batch 17 records for agile methodologies, causal attribution
  resistance, chain of thought, circle of competence, complexity resistance,
  endowment effect, latticework, logical fallacies, mental models of reality,
  meta-cognitive reflection, perceptual learning, scaffolding, System 1,
  System 2, Tier-2 High Value, and time-tested validation. It compiles
  draft/review-only v18, preserves `32` absence records, and leaves `0`
  runtime models graph-only in the draft reviewed artifact. Decision label:
...
- Next-session handover is captured in
  ...
  `full_reviewed_source_backed_coverage_complete`; do not build runtime packet
  ...
  Full reviewed source-backed coverage is complete in v18. The next slice
  should audit full-corpus quality and stress packet shape before any runtime
  promotion.
...
or Decision Pressure trace machinery. PR13-PR47 are complete or in review.
...
PR41 then audited the remaining 112 graph-only models after v9 and selected
risk controls / reversibility / failure containment as the next controlled
enrichment family.
PR42 then executed that controlled risk/reversibility batch for 12 named
graph-only models, compiled v10 as draft/review-only material, and preserved
absence records as first-class corpus evidence.
PR43 then compared one explicit 12-card risk/reversibility packet against v9
and v10 and found the v10 reviewed cards improve handoff material without
increasing candidate count or selecting final pressure.
PR44 then audited the remaining 100 graph-only models after v10 and selected
frame correction / metacognitive blind-spot discipline as the next controlled
enrichment family.
PR45 then executed that controlled frame-correction batch for 12 named
graph-only models, compiled v11 as draft/review-only material, and preserved
absence records as first-class corpus evidence. It explicitly does not create
...
22. Use `research/v9-graph-only-priority-audit-2026-05-07.md` as the current
    after-v9 graph-only priority audit.
23. Use `research/pr42-controlled-risk-reversibility-enrichment-report-2026-05-07.md`
    as the current controlled v10 enrichment report.
24. Use `research/reasoning-substrate-v10-packet-usefulness-review-2026-05-07.md`
    as the current v10 packet usefulness review.
25. Use `research/v10-graph-only-priority-audit-2026-05-07.md` as the current
    after-v10 graph-only priority audit.
26. Use `research/pr45-controlled-frame-correction-enrichment-report-2026-05-07.md`
    as the current controlled v11 enrichment report.
27. Use `research/decision-pressure-product-doctrine-2026-05-06.md` as the
   product north star: broad intake, disciplined output.
28. Use `research/enriched-mental-model-packet-strategy-2026-05-06.md` as the
   architecture line: pull shelves, enrich cards, let the LLM reason.
29. Preserve PR19's runtime-dormant `decision_pressure_trace` contract before
   any UI or runtime work.
30. Treat PR20/PR21/PR22 adapter work as review infrastructure, not live product.
31. Treat PR23's generalization readout as directional product evidence, not
   deterministic case logic.
32. Treat PR25 as dormant enrichment placement, not runtime packet production.
33. Treat PR26 as source custody, not v4 extraction.
34. Treat PR27 as fixture usefulness evidence, not runtime packet production.
35. Treat PR28 as controlled extraction quality evidence, not v5 runtime
    promotion or broad extraction permission.
36. Treat PR29 as handoff-depth evidence, not final-answer evidence or
    permission for deterministic pressure selection.
37. Treat PR30 as receiver-review ergonomics, not a product renderer, UI, memo,
    package function, or semantic selector.
38. Treat PR31 as a capability audit, not extraction or runtime promotion.
39. Treat PR32 as controlled capability-gap enrichment, not v6 runtime
    promotion, corpus completion, or proof that another batch should begin.
40. Treat PR33 as packet handoff usefulness evidence, not final-answer evidence,
    live packet production, or runtime permission.
41. Treat PR34 as controlled communication/competition enrichment, not v7
    runtime promotion, corpus completion, or proof that another batch should
    begin.
42. Treat PR35 as packet handoff usefulness evidence, not final-answer evidence,
    live packet production, or runtime permission.
43. Treat PR36 as controlled trust/negotiation enrichment, not v8 runtime
    promotion, corpus completion, or proof that another batch should begin.
44. Treat PR37 as packet handoff usefulness evidence, not final-answer evidence,
    live packet production, or runtime permission.
45. Treat PR38 as a graph-only priority audit, not extraction.
46. Treat PR39 as controlled execution/follow-through enrichment, not v9
    runtime promotion, corpus completion, or proof that another batch should
    begin.
47. Treat PR40 as packet handoff usefulness evidence, not final-answer
    evidence, live packet production, or runtime permission.
48. Treat PR41 as a graph-only priority audit, not extraction.
49. Treat PR42 as controlled risk/reversibility enrichment, not v10 runtime
    promotion, corpus completion, or proof that another batch should begin.
50. Treat PR43 as packet handoff usefulness evidence, not final-answer
    evidence, live packet production, or runtime permission.
51. Treat PR44 as a graph-only priority audit, not extraction.
...
55. Treat PR48 as controlled adaptive exploration enrichment, not v12 runtime
    ...
    completion, or proof that another batch should begin.
56. Treat PR49 as controlled learning / skill-acquisition enrichment, not v13
    runtime promotion, prompt mechanics, deterministic mastery classification,
    or proof that Python can decide what someone should learn next.
57. Treat PR50 as controlled quantitative inference / distributional reasoning
    enrichment, not v14 runtime promotion, prompt mechanics, deterministic
    statistical routing, model-output authority, or proof that Python can
    decide which numeric claim should govern the final answer.
58. Treat PR51 as controlled self-regulation / bias-calibration enrichment,
...
61. Treat PR54 as controlled final graph-only enrichment, not v18 runtime
    promotion, prompt mechanics, deterministic reasoning-mode routing,
    educational diagnosis, psychological diagnosis, or proof that full coverage
    means Python can choose final judgment.
62. Do not start runtime, prompt changes, lane rewrites, broad extraction,
    Batch 3b, or user-facing promotion from this slice.
63. Preserve coverage honesty and no-casuistry rails.
64. Do not run more paid model calls for this slice.
65. Do not start UI, trace-fixture stress tests, live route-trace adapter work,
    Batch 3b, prompt changes, or runtime work by default.

Controlled extraction reached full reviewed source-backed coverage in PR54.
v18 now covers all 222 runtime models as a draft/review-only artifact, with
258 affordances and 429 absence records. The next slice should not extract more
by default; it should audit full-corpus quality and stress packet shape before
