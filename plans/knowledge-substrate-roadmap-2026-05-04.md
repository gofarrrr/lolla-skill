...
- PR42 controlled risk/reversibility enrichment is done for 12 graph-only
  ...
  pressure selection.
- PR43 v10 risk/reversibility packet usefulness review is done for one
  explicit 12-card risk/reversibility nomination set. The same packet goes from
  0 reviewed / 12 graph-only cards under v9 to 12 reviewed / 0 graph-only cards
  under v10, without changing candidate count or producing final
  pressure/user-facing output.
- PR44 after-v10 graph-only priority audit is done. It reviews the remaining
  100 graph-only runtime models after v10 and recommends frame correction /
  metacognitive blind-spot discipline as the next controlled enrichment family
...
- PR46 v11 frame-correction packet usefulness review is done for one explicit
  12-card frame-correction nomination set. The same packet goes from 0
  reviewed / 12 graph-only cards under v10 to 12 reviewed / 0 graph-only cards
  under v11, without changing candidate count or producing final
  pressure/user-facing output.
- PR47 after-v11 graph-only priority audit is done. It reviews the remaining
  88 graph-only runtime models after v11 and recommends adaptive exploration /
  option generation / synthesis discipline as the next controlled enrichment
...
- PR49 controlled learning / skill-acquisition enrichment is done for 12
  graph-only models selected from the remaining v12 graph-only set. It adds
...
- PR51 controlled self-regulation / bias-calibration enrichment is done for 12
  graph-only models selected from the remaining v14 graph-only set. It adds
...
- PR53 controlled economic / systems structure enrichment is done for 12
  graph-only models selected from the remaining v16 graph-only set. It adds
  Batch 16 records and compiles draft/review-only v17. The records are
...
- v12 corpus shape: `146` reviewed records, `182` affordances, `277` absence
  ...
  still `draft_review_only` and not runtime-promoted.
- Compiled v13 artifact exists at
...
- v14 corpus shape: `170` reviewed records, `206` affordances, `325` absence
  ...
  still `draft_review_only` and not runtime-promoted.
- Compiled v15 artifact exists at
...
- v16 corpus shape: `194` reviewed records, `230` affordances, `373` absence
  ...
  still `draft_review_only` and not runtime-promoted.
- Compiled v17 artifact exists at
...
- PR43 v10 risk/reversibility packet usefulness review is captured in
  `research/reasoning-substrate-v10-packet-usefulness-review-2026-05-07.md`:
  decision label `v10_risk_packet_handoff_useful`. A v9/v10 packet comparison
  using the same explicit nominations shows v10 adds useful handoff depth for
  commitment sizing, reversibility decay, fallback independence, weak-signal
  triggers, adversarial failure chains, nonlinear loops, threshold evidence,
  plausible cascades, resilience-over-precision, make-or-break interactions,
  critical-mass density, and loss-frame distortion without changing candidate
  count.
- PR44 after-v10 graph-only priority audit is captured in
  `research/v10-graph-only-priority-audit-2026-05-07.md`: decision label
  `v10_graph_only_priority_audit_complete`. It does not extract records. It
  selects frame correction / metacognitive blind-spot discipline as the best
  next controlled enrichment family, with a 12-model PR45 target set, because
...
- PR45 controlled frame-correction enrichment is captured in
  ...
  deterministic reasoning-mode routing.
- PR46 v11 frame-correction packet usefulness review is captured in
  `research/reasoning-substrate-v11-packet-usefulness-review-2026-05-07.md`:
  decision label `v11_frame_correction_packet_handoff_useful`. A v10/v11
  packet comparison using the same explicit nominations shows v11 adds useful
  handoff depth for missing reality gaps, claim/evidence checks, plausible
  counterfactuals, bounded next questions, mode-fit handoff, reframing,
  framework blindness, familiar-solution lock-in, dialectical synthesis,
  inward bias checks, false precision, and missing denominators without
  changing candidate count.
- PR47 after-v11 graph-only priority audit is captured in
  `research/v11-graph-only-priority-audit-2026-05-07.md`: decision label
  `v11_graph_only_priority_audit_complete`. It does not extract records. It
  selects adaptive exploration / option generation / synthesis discipline as
  the best next controlled enrichment family, with a 12-model PR48 target set,
...
- PR48 controlled adaptive exploration enrichment is captured in
  ...
  and merge rules.
- PR49 controlled learning / skill-acquisition enrichment is captured in
  `research/pr49-controlled-learning-skill-enrichment-report-2026-05-07.md`:
  decision label `controlled_learning_skill_enrichment_ready`. Twelve graph-only
  models gained reviewed Batch 12 records with 12 affordances and 24 absence
...
- PR51 controlled self-regulation / bias-calibration enrichment is captured in
  `research/pr51-controlled-self-regulation-bias-enrichment-report-2026-05-07.md`:
  decision label `controlled_self_regulation_bias_enrichment_ready`. Twelve
  graph-only models gained reviewed Batch 14 records with 12 affordances and 24
  absence records, compiled as draft/review-only v15. The cards bind
...
- PR53 controlled economic / systems structure enrichment is captured in
  ...
  Twelve graph-only models gained reviewed Batch 16 records with 12 affordances
  and 24 absence records, compiled as draft/review-only v17. The cards bind
  market, scale, specialization, adaptation, emergent-order, role-fit,
