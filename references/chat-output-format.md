...
- **Stakeholder assumption material.** If the optional stakeholder check surfaces a material correction, include only `stakeholder_assumption_check.chat_actors` as a concrete divergence inside `### Pressure Check`. Name the actor dependency and the plan change; do not name the checker. Do not add a separate stakeholder section. Do not consume `critical_actors` as chat material, because that inspection payload may include speculative or duplicate entries. Do not write speculative psychology (*"he likely feels..."*, *"she probably wants..."*) unless the transcript grounds it and the plan changes because of it.
