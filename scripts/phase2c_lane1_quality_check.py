#!/usr/bin/env python3
"""Phase 2c Lane 1 quality check over the conversation-first runtime.

Runs `run_extract.py` once per case (shared input), then runs `run_pipeline.py`
N times on the ConversationContext path. Parses Lane 1 outputs
(detected_tendencies, delta_card) AND downstream cascade metrics (Lane 2/3/4
sizes) from each result.
