#!/usr/bin/env python3
"""Export archived Lolla runs into a semantic-coverage corpus."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build a local JSONL survey of semantic coverage from archived "
            "Lolla runs. The export is read-only, does not score answer "
            "quality, and does not call models."
        )
    )
    parser.add_argument(
        "archive_root",
    ...
    parser.add_argument(
        "--out",
        type=Path,
        help="Write one compact semantic-coverage corpus record per run as JSONL.",
    )
    parser.add_argument(
        "--manifest-out",
        type=Path,
    ...
    if args.manifest_only or not args.out:
        ...
        print(
            f"exported {len(records)} semantic coverage records to {args.out}"
