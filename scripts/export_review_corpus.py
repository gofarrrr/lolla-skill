#!/usr/bin/env python3
"""Export archived Lolla runs into a deterministic human-review corpus."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build a local JSONL Lolla review corpus and deterministic manifest "
            "from archived run artifacts. The export does not score advice "
    ...
    parser.add_argument(
        "--out",
        type=Path,
        help="Write one compact corpus record per run as JSONL.",
    )
    parser.add_argument(
        "--manifest-out",
        type=Path,
    ...
    if args.manifest_only or not args.out:
        ...
        print(
            f"exported {len(records)} review corpus records to {args.out}"
