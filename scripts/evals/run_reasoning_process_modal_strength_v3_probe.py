#!/usr/bin/env python3
"""Run one frozen fresh-case modal-strength position probe via Gemini/OpenRouter."""
...
CONTRACT_SCHEMA = "lolla.reasoning_process_modal_strength_v3_probe_contract.v1"
AUTH_SCHEMA = "lolla.reasoning_process_modal_strength_v3_probe_authorization.v1"


def validate_contract(contract: dict, contract_path: Path) -> dict:
    if contract.get("schema_version") != CONTRACT_SCHEMA:
        raise RuntimeError("unexpected modal-strength v3 probe contract")
    if contract.get("status") != "frozen_before_one_fresh_provider_call":
        raise RuntimeError("modal-strength v3 probe contract is not frozen")
    ...
    if _sha(packet_path) != job["packet_sha256"]:
        raise RuntimeError("modal-strength probe packet drifted")
    wrapper = _load(packet_path)
    prompts = build_shard_prompts_v3(wrapper)
    schema = shard_response_schema_v3(job["view_kind"])
    observed = {
    ...
    if any(job[key] != value for key, value in observed.items()):
        raise RuntimeError("modal-strength probe prompt or schema drifted")
    if (
        job["case_id"] != "amb1-case03-creative-partnership"
        or job["mechanism"] != "chronological_shard_modal_strength_v3"
    ...
    if (
        ...
        raise RuntimeError("modal-strength probe route or failure policy drifted")
    if contract["budget"] != {
        ...
        raise RuntimeError("modal-strength probe budget drifted")
    return {
        "status": "modal_strength_v3_probe_contract_valid",
...
def validate_authorization(value: dict, *, contract: dict, contract_path: Path) -> None:
    ...
    if value != expected:
        raise RuntimeError("modal-strength v3 probe authorization drifted")
...
def main() -> int:
    ...
    if output.exists():
        raise RuntimeError("modal-strength probe output already exists")
    ...
    result = {
        "schema_version": "lolla.reasoning_process_modal_strength_v3_probe_result.v1",
