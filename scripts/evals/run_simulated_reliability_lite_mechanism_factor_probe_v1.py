#!/usr/bin/env python3
"""Run one frozen factored-user-status plus assistant-coverage mechanism probe."""
...
SCHEMA = "lolla.simulated_reliability_lite_mechanism_factor_contract.v1"
AUTH_SCHEMA = "lolla.simulated_reliability_lite_mechanism_factor_authorization.v1"
RESULT_SCHEMA = "lolla.simulated_reliability_lite_mechanism_factor_result.v1"
...
def _first(contract: Mapping[str, Any]) -> dict[str, Any]:
    packet = build_user_factor_packet_v1(
        parent_packet=_parent(contract), mechanism_id=contract["mechanism_id"]
    )
    return {
        "packet": packet,
        "prompts": build_user_factor_prompts_v1(packet),
        "response_schema": user_factor_response_schema_v1(contract["mechanism_id"])
    }


def request_attestation(contract: Mapping[str, Any]) -> dict[str, Any]:
    first = _first(contract)
    return {
        "user_factor_packet_sha256": value_sha(first["packet"]),
        "role_record_count": len(first["packet"]["role_records"]),
        "user_factor_system_prompt_sha256": first["prompts"]["system_prompt_sha256"],
        "user_factor_user_prompt_sha256": first["prompts"]["user_prompt_sha256"],
        "user_factor_response_schema_sha256": value_sha(first["response_schema"]),
...
def _validate(contract_path: Path, authorization_path: Path) -> dict[str, Any]:
    ...
    if contract.get("schema_version") != SCHEMA:
        raise V1RunError("unexpected mechanism-factor contract schema")
    if contract.get("status") != "frozen_before_one_factored_transfer_probe":
        raise V1RunError("mechanism-factor contract is not frozen")
    if contract.get("budget", {}).get("maximum_provider_calls") != 2:
        raise V1RunError("mechanism-factor call ceiling drifted")
    ...
    if request_attestation(contract) != contract["request_attestation"]:
        raise V1RunError("mechanism-factor request bytes drifted")
    ...
    if authorization != expected:
        raise V1RunError("mechanism-factor authorization drifted")
    return contract
...
def run(
    ...
    runtime["seeds"]["factor_user"] = contract["seeds"]["user_factor"]
    runtime["seeds"]["factor_coverage"] = contract["seeds"]["assistant_coverage"]
    ...
    raw_user = call_fn(
        ...
        repeat_id="factor_user",
        contract=runtime,
        prompts=first["prompts"],
        schema=first["response_schema"],
        schema_name="lolla_v1_mechanism_user_factor",
        compile_candidate=lambda candidate: compile_user_factor_response_v1(
            response=candidate, packet=first["packet"], producer_id=operator["model"]
        )
    )
    user = _row(raw_user)
    coverage = None
    ...
    if isinstance(user.get("compiled"), Mapping):
        coverage_packet = build_assistant_coverage_packet_v1(
            parent_packet=parent, user_status=user["compiled"]["assessment"]
        )
        write(output / "assistant-coverage-packet.json", coverage_packet)
        prompts = build_assistant_coverage_prompts_v1(coverage_packet)
        raw_coverage = call_fn(
            output=output,
            ordinal=2,
            task_id="mechanism_assistant_coverage",
            case_id=contract["case_id"],
            repeat_id="factor_coverage",
            contract=runtime,
            prompts=prompts,
            ...
            compile_candidate=lambda candidate: compile_assistant_coverage_response_v1(
                response=candidate, packet=coverage_packet, producer_id=operator["model"]
            )
        )
        coverage = _row(raw_coverage)
    ...
    costs = [float(row["provider_reported_cost_usd"]) for row in rows if row.get("provider_reported_cost_usd") is not None]
    total = round(sum(costs), 12) if len(costs) == calls else None
    report = {
        "schema_version": RESULT_SCHEMA,
        "status": "factored_mechanism_preserved_source_review_required",
        "run_id": contract["run_id"],
        "case_id": contract["case_id"],
        "mechanism_id": contract["mechanism_id"],
        "user_factor_task": user,
...
def main() -> int:
    ...
    if output.exists():
        raise V1RunError("mechanism-factor output path must not exist")
