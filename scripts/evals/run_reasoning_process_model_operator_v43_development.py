#!/usr/bin/env python3
"""Compare two provider-pinned pairs on the prompt-only v4.3 development contract."""
...
CONTRACT_SCHEMA = "lolla.reasoning_process_model_operator_v43_development_contract.v1"
AUTH_SCHEMA = "lolla.reasoning_process_model_operator_v43_development_authorization.v1"


def validate_contract(contract: dict, contract_path: Path) -> dict:
    if contract.get("schema_version") != CONTRACT_SCHEMA:
        raise RuntimeError("unexpected v4.3 model/operator development contract")
    if contract.get("status") != "frozen_before_two_v43_synthetic_development_calls":
        raise RuntimeError("v4.3 development contract is not frozen")
    ...
    if _sha(packet_path) != contract["packet"]["sha256"]:
        raise RuntimeError("v4.3 development packet drifted")
    ...
    if any(contract["request_contract"][key] != value for key, value in observed.items()):
        raise RuntimeError("v4.3 prompt or schema drifted")
    expected = {
        ("deepseek/deepseek-v4-flash", "alibaba"),
        ("z-ai/glm-5.2", "deepinfra"),
    }
    jobs = contract["jobs"]
    if len(jobs) != 2 or {(job["model"], job["provider_slug"]) for job in jobs} != expected:
        raise RuntimeError("v4.3 development pairs drifted")
    if contract["budget"]["maximum_provider_calls"] != 2:
        raise RuntimeError("v4.3 development budget drifted")
    if not all(contract["custody"].values()):
        raise RuntimeError("v4.3 development custody drifted")
    ...
    if config != {
        ...
        raise RuntimeError("v4.3 development call configuration drifted")
    return {
        "status": "model_operator_v43_development_contract_valid",
...
def validate_authorization(value: dict, *, contract: dict, contract_path: Path) -> None:
    expected = {
        ...
        "status": "authorized_once_after_provider_free_v43_alignment_gate",
        "contract_path": str(contract_path.relative_to(ROOT)),
    ...
    if value != expected:
        raise RuntimeError("v4.3 development authorization drifted")
...
def main() -> int:
    ...
    if not output_dir.is_dir() or aggregate_path.exists():
        raise RuntimeError("v4.3 development output already exists or directory is absent")
    ...
    for index, job in enumerate(contract["jobs"], start=1):
        ...
        if started_path.exists() or result_path.exists():
            raise RuntimeError(f"v4.3 call {index} already started; rerun forbidden")
        _write(
            started_path,
            {
        ...
        result = run_model_operator_job(
            ...
            response_schema_name="lolla_reasoning_process_stance_object_v43_development",
        )
        _write(result_path, result)
        results.append(result)
    aggregate = {
        "schema_version": "lolla.reasoning_process_model_operator_v43_development_result.v1",
        "status": "two_v43_synthetic_development_calls_preserved",
        "run_id": contract["run_id"],
        "calls": results,
