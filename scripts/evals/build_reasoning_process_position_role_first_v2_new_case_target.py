#!/usr/bin/env python3
"""Compile the pre-written source-review target for the new role-first v2 case."""
...
def build(output: Path) -> dict:
    ...
    for role in ROLE_ORDER:
        ...
        roles[role] = compile_position_role_response_v2(
            response=target["role_responses"][role],
            packet=packet,
            producer_kind="source_reviewer",
            producer_id="pre-execution-new-case-target",
        )
    relation_packet = build_position_relation_packet_v2(role_compiled_by_role=roles)
    relationship = target["relationship_target"]
    role_ids = {
        role: {
            item["source_record_index"]: item["role_record_id"]
            for item in roles[role]["observations"]
        }
        for role in ROLE_ORDER
    }
    relation_response = {
        ...
        "records": [
            {
                "status": relationship["status"],
                "starting_role_record_id": role_ids["starting"][relationship["starting_source_record_index"]],
                "current_role_record_id": role_ids["current"][relationship["current_source_record_index"]],
                "qualification_role_record_id": role_ids["qualification"][relationship["qualification_source_record_index"]],
    ...
    relation = compile_position_relation_response_v2(
    ...
    quarantined = sum(
        ...
        for compiled in [*roles.values(), relation]
        for item in compiled["records"]
    )
    gate = (
        all(len(roles[role]["observations"]) == 1 for role in ROLE_ORDER)
        and len(relation["observations"]) == 1
        and quarantined == 0
        and joined["status"] == "position_role_first_join_complete"
    )
    fixture = {
        "schema_version": "lolla.reasoning_process_position_role_first_new_case_fixture.v1",
    ...
    report = {
        "schema_version": "lolla.reasoning_process_position_role_first_new_case_target_report.v1",
        ...
        "compiled_target_sha256": hashlib.sha256(fixture_path.read_bytes()).hexdigest(),
        "admitted_role_record_count": sum(len(value["observations"]) for value in roles.values()),
        "admitted_relation_record_count": len(relation["observations"]),
