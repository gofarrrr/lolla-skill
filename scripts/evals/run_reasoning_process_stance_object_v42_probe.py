#!/usr/bin/env python3
"""Run one frozen reserved-case stance-object v4.2 probe via Gemini/OpenRouter."""
...
CONTRACT_SCHEMA = "lolla.reasoning_process_stance_object_v42_probe_contract.v1"
AUTH_SCHEMA = "lolla.reasoning_process_stance_object_v42_probe_authorization.v1"


def validate_contract(contract: dict, contract_path: Path) -> dict:
    if contract.get("schema_version") != CONTRACT_SCHEMA:
        raise RuntimeError("unexpected stance-object v4.2 probe contract")
    if contract.get("status") != "frozen_before_one_reserved_provider_call":
        raise RuntimeError("stance-object v4.2 probe contract is not frozen")
    ...
    if _sha(packet_path) != job["packet_sha256"]:
        raise RuntimeError("stance-object v4.2 probe packet drifted")
    wrapper = _load(packet_path)
    prompts = build_shard_prompts_v42(wrapper)
    schema = shard_response_schema_v42(job["view_kind"])
    observed = {
    ...
    if any(job[key] != value for key, value in observed.items()):
        raise RuntimeError("stance-object v4.2 prompt or schema drifted")
    if (
        job["case_id"] != "amb2-case02-community-space"
        or job["mechanism"] != "chronological_shard_stance_object_v42"
        or job["view_kind"] != "position_and_decision_trajectory"
    ):
        raise RuntimeError("reserved-case or mechanism selection drifted")
    preflight = _load(ROOT / contract["google_schema_preflight"]["path"])
    if (
        preflight.get("status") != "pass"
        or preflight.get("v42", {}).get("native_schema_status") != "pass"
        or preflight.get("v42", {}).get("schema_sha256") != job["response_schema_sha256"]
    ):
        raise RuntimeError("Google native Schema preflight drifted")
    ...
    if (
        ...
        raise RuntimeError("stance-object v4.2 route or failure policy drifted")
    if contract["budget"] != {
        ...
        raise RuntimeError("stance-object v4.2 budget drifted")
    return {
        "status": "stance_object_v42_probe_contract_valid",
...
def validate_authorization(value: dict, *, contract: dict, contract_path: Path) -> None:
    expected = {
        ...
        "status": "authorized_once_after_sdk_adversarial_and_cold_reader_gates",
        "contract_path": str(contract_path.relative_to(ROOT)),
    ...
    if value != expected:
        raise RuntimeError("stance-object v4.2 authorization drifted")
...
def main() -> int:
    ...
    if output.exists():
        raise RuntimeError("stance-object v4.2 probe output already exists")
    ...
    call = run_structured_shard_job(
        ...
        job=contract["job"],
        snapshot=snapshot,
        build_prompts=build_shard_prompts_v42,
        build_schema=shard_response_schema_v42,
        compile_response=compile_shard_response_recordwise_v42,
        call_schema="lolla.reasoning_process_stance_object_v42_probe_call.v1",
        response_schema_name="lolla_reasoning_process_stance_object_v42",
    )
    result = {
        "schema_version": "lolla.reasoning_process_stance_object_v42_probe_result.v1",
        "status": "one_reserved_position_probe_call_preserved",
        "run_id": contract["run_id"],
        "call": call,
