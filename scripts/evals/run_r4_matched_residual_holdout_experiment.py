#!/usr/bin/env python3
"""Run the frozen matched R4 experiment only after exact founder authorization."""
...
DEFAULT_CONTRACT = ROOT / "docs/evals/lolla-r4-matched-residual-holdout-contract-v1.json"
CONTRACT_SCHEMA = "lolla.r4_matched_residual_holdout_contract.v1"
AUTH_SCHEMA = "lolla.r4_matched_residual_holdout_authorization.v1"


class R4MatchedResidualRunError(RuntimeError):
...
def _load(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise R4MatchedResidualRunError(f"expected JSON object: {path}")
    return value
...
def validate_contract(path: Path = DEFAULT_CONTRACT) -> dict[str, Any]:
    """Validate only the immutable execution-visible package."""

    path = path.resolve()
    if path != DEFAULT_CONTRACT.resolve():
        raise R4MatchedResidualRunError("only the frozen default contract is runnable")
    contract = _load(path)
    if (
        contract.get("schema_version") != CONTRACT_SCHEMA
        or contract.get("status")
        != "provider_free_matched_holdout_frozen_no_authorization"
        or contract.get("run_id") != "lolla-r4-matched-residual-holdout-v1"
    ):
        raise R4MatchedResidualRunError("matched holdout contract drifted")
    ...
    if (
        ...
        raise R4MatchedResidualRunError("decision boundary drifted")
    ...
    if any(budget.get(key) != value for key, value in expected_budget.items()):
        raise R4MatchedResidualRunError("execution budget drifted")
    if contract.get("operator") != {
        ...
        raise R4MatchedResidualRunError("operator boundary drifted")
    ...
    if (
        ...
        raise R4MatchedResidualRunError("eight-call plan drifted")
    if sum(row.get("arm", "").startswith("A_") for row in call_plan) != 4:
        raise R4MatchedResidualRunError("arm A allocation drifted")
    if sum(row.get("arm", "").startswith("B_") for row in call_plan) != 4:
        raise R4MatchedResidualRunError("arm B allocation drifted")
    ...
    if (
        ...
        raise R4MatchedResidualRunError("execution manifest drifted")
    ...
    if (
        ...
        raise R4MatchedResidualRunError("execution-visible boundary drifted")
    for row in manifest.get("files", []):
        ...
        if (
            ...
            raise R4MatchedResidualRunError(f"execution artifact drifted: {row['path']}")
    for case in contract.get("cases", []):
        for prefix in ("source", "prior"):
            artifact = ROOT / case[f"{prefix}_path"]
            if not artifact.is_file() or _file_sha(artifact) != case[f"{prefix}_sha256"]:
                raise R4MatchedResidualRunError(
                    f"{prefix} artifact drifted: {case.get('case_id')}"
                )
    history = contract.get("frozen_history", {})
    current = {
        "v1_module_sha256": _file_sha(ROOT / "engine/system_b/r4_complementary_readers.py"),
        "v2_module_sha256": _file_sha(ROOT / "engine/system_b/r4_semantic_distinction.py"),
        "residual_module_sha256": _file_sha(ROOT / "engine/system_b/r4_residual_task.py"),
    ...
    if any(history.get(key) != value for key, value in current.items()):
        raise R4MatchedResidualRunError("historical prompt or schema drifted")
    ...
    if (
        ...
        raise R4MatchedResidualRunError("future runner drifted")
    for row in call_plan:
        preview = _load(ROOT / row["request_preview_path"])
        if preview.get("body_sha256") != row.get("request_body_sha256"):
            raise R4MatchedResidualRunError("request preview drifted")
    return contract


def expected_authorization(*, contract: Mapping[str, Any]) -> dict[str, Any]:
    """Return the one exact object a separate founder action would have to match."""
...
def validate_authorization(
    ...
    if _load(authorization_path.resolve()) != expected_authorization(contract=contract):
        raise R4MatchedResidualRunError("authorization drifted")
...
def _load_env(path: Path) -> None:
    if not path.is_file():
        raise R4MatchedResidualRunError("environment file does not exist")
...
def _openrouter_transport(*, endpoint: str) -> Callable[[dict[str, Any]], bytes]:
    """Create the single-attempt HTTP boundary after exact authorization."""

    api_key = os.getenv("LOLLA_OPENROUTER_API_KEY") or os.getenv("OPENROUTER_API_KEY")
    if not api_key:
        raise R4MatchedResidualRunError("OpenRouter API key is absent")
...
def execute(
    *,
    ...
    if dict(contract) != frozen:
        raise R4MatchedResidualRunError("in-memory contract drifted")
    validate_authorization(authorization_path, contract=frozen)
    output = output.resolve()
    if output.exists():
        raise R4MatchedResidualRunError("execution output path already exists")
    ...
    for plan in frozen["call_plan"]:
        ...
        if request_sha != plan["request_body_sha256"]:
            raise R4MatchedResidualRunError("request body changed after validation")
        ...
        finish_reason = str(choice.get("finish_reason", "")) if isinstance(
            choice, Mapping
        ) else ""
        ...
        if operational_status == "candidate_ready_for_admission" and candidate is not None:
            ...
            try:
                if arm.startswith("A_"):
                    ...
                    raise R4MatchedResidualRunError("undeclared arm")
        ...
        call_result = {
            **base,
            ...
            "candidate_sha256": value_sha256(candidate) if candidate is not None else None,
    ...
    result = {
        "schema_version": "lolla.r4_matched_residual_execution_result.v1",
