...
1. **Skill directory location** — resolves the bundled skill root from `scripts/skill/setup.sh` itself, so a repository clone, copied installation, or symlinked installation does not depend on the caller's current directory or another checkout.
...
previous terminal state afterward. This is mechanical custody, not a
repository edit or semantic judgment. The skill must not use Apply Patch or a
file editor for this step.
...
**Long conversation handling:** The complete available prose conversation is always written as the authoritative `conversation.txt`, even above 100 turns. If the captured source exceeds the extraction boundary's 80K-character processing cap, `run_extract.py` writes a separately named `conversation_processing_view.txt` containing the first 3 and last 15 parsed turn blocks plus an omission marker. Its JSON metadata records both hashes, both lengths, and exact omission counts. The derivative is never labeled authoritative.
...
- `capture_manifest` — actual vs. declared turn counts (user, assistant) and authoritative character length. It describes source capture only. Processing compaction is recorded separately under `conversation_processing_view`, whose metadata includes `status: partial`, both artifact hashes and lengths, and exact omission fields. This prevents bounded processing from being mislabeled as source loss while still making the extraction limitation explicit.
- `capture_adequacy` — compact deterministic metadata with schema `lolla.capture_adequacy.v0`. It records `good` / `warn` / `critical`, the capture strategy, declared/captured/omitted counts, captured windows, omitted windows, risk flags, and short notes. This makes capture loss inspectable; it does not semantically reconstruct omitted turns or change the long-conversation capture strategy.
...
1. **Run-health line, conditional.** If `run_health.overall` is not `"healthy"` and the issue affects trust in the run, open with one plain note. Material issues include `capture_degraded`, `capture_critical`, `substrate_empty`, `no_fingerprint`, `quote_fabrication`, `extraction_processing_view_partial`, `lane3_all_dropped`, `bullshit_index_partial`, and `provider_call_terminal_loss`. For the processing-view issue, say that the full source is preserved and initial extraction omitted declared middle turns; do not say the conversation itself was truncated. For provider-call loss, name the affected stage/check and say the call ended without usable output and was not retried. Clean runs say nothing about health.
...
After launching the Observatory, the skill archives the run's core artifacts into a persistent case folder under `~/.local/share/lolla/runs/` (or `$LOLLA_ARCHIVE_DIR`) so the run survives `/tmp` cleanup and stays accessible for later review, memo re-rendering, `scripts/stability_check.py`, `scripts/compare_archived_runs.py`, or `scripts/export_reasoning_trace_dataset.py` analysis. Before copying, `scripts/archive_run.py` finalizes V60 ledger telemetry, product-output hygiene, and live-output hygiene. Archive-time live-output finalization treats `/tmp/lolla_<run_id>_live_transcript.txt` as a manual artifact by default: detected leaks are unsafe, but no-leak manual capture is `not_checked`, not proof of the full console surface. When the Step 9 finalizer receives `--trusted-transcript /path --require-live-output-clean`, it appends the final receipt to that complete capture, syncs it into `/tmp/lolla_<run_id>_live_transcript.txt`, and only then archives; the resulting `live_output_health: clean` is hash-backed against the archived transcript. It copies 24 core/optional files—including authoritative conversation, separately declared processing view and metadata, extraction, provider budget, graph-survival ledger, result, revised answer, memo, private ledgers, live transcript, operator/events evidence, reviews, and optional control input—into `{archive_root}/{case_id}/{run_id}/`. Missing optional artifacts are skipped gracefully, while required active ledgers fail their finalizer before archive. The archive also generates `agent_result.json`, a compact `lolla_agent_result.v2` handoff for machine callers; optional `control_result.json` when `control_input.json` was supplied; `extraction_adequacy_report.json`, a deterministic report showing where the current `conversation.txt -> extraction.json -> ConversationContext -> ConversationIR` chain preserves, weakens, or loses source provenance; `evaluation.json`, a deterministic run-readiness receipt for artifact/schema/custody/health consistency; and `reasoning_trace.json`, a local-only custody manifest that records artifact paths, SHA-256 hashes, health, capture adequacy, extraction adequacy, optional control-plane references, usage, reasoning-lens IDs, budget-suppressed lens summaries, candidate commitment classifications, user usefulness state, outcome review state, model-call telemetry, and trace-adequacy status without duplicating raw conversation or memo text. Before completion, archive/case/run directories are forced to `0700` and manifests plus all archived files to `0600`. `/tmp` originals are not moved or deleted; run setup and direct extraction/pipeline writers keep fresh run-scoped files owner-only, and convenience copies of generated result artifacts remain available to the active caller.
