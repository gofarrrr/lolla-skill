...
| Run identity and recovery | `run_id`, `LOLLA_EXPECTED_RUN_ID`, `run_events` | `run_state.py`, `record_run_event.py`, `SKILL.md`, `run_extract.py`, `run_pipeline.py`, `archive_run.py` | Collision-resistant run IDs, run-specific env files, expected-run validation before major model calls/writes, and an append-only recovery ledger for restarts, pins, aborts, and pointer rewrites. |
| Archive custody | `graph_survival_report.*` + `reasoning_trace.json` | `archive_run.py`, `graph_survival_report.py`, `reasoning_trace.py`, `reasoning_trace_dataset.py` | Local archive layer that preserves artifact hashes, health, usage, model-call telemetry, selected/suppressed/budget-suppressed lenses, candidate commitments, usefulness/outcome review state, and trace adequacy without duplicating raw text in the trace. |
...
| **Run-capture and reasoning-trace hardening (2026-06-23)** | Run IDs now use timestamp plus random suffix; `SKILL.md` writes a run-specific env file and treats `/tmp/lolla_latest_env.sh` as a pointer only; guarded scripts validate `LOLLA_EXPECTED_RUN_ID` and artifact path ownership before expensive work; recovery actions can be recorded in `run_events.json`; live-output hygiene detects cross-case `## Updated position` contamination; structural coverage adds `coverage_quality`; archive matching uses conversation hash before decision fingerprint; `reasoning_trace.json` carries budget-suppressed lenses, candidate-commitment classification, run events, user usefulness state, and outcome review state. | Parallel and repeated `/lolla` runs exposed a new class of failure: the reasoning engine could be correct while the orchestration surface mixed scratch state from a previous case. The fix is not "be more careful"; it is explicit run identity, expected-run guards, semantic live-output checks, and archive custody that records recovery rather than hiding it. |
...
- Live-output hygiene: `live_output_health` (`clean`, `unsafe`, `missing`, or `not_checked`), `live_output_leak_count`, and leak details for the Claude Code live transcript artifact. `clean` requires a trusted complete transcript; a clean manual artifact is `not_checked`.
