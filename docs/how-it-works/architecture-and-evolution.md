...
| Dormant shadow experiment | `pre_step6_shadow_portfolio` | `pre_step6_shadow_portfolio.py`, `run_pipeline.py`, `archive_run.py`, `serve_result.py` | Default-off, shadow-only evidence recorder for cached pre-Step-6 card decks. It never generates live decks and never changes visible output. |
...
There is also a default-off pre-Step-6 shadow portfolio hook. It is a research integration boundary, not live product behavior: `--pre-step6-portfolio shadow` only records whether a cached deck would have been eligible under the proposed policy and exposes that record in archive and Observatory `/audit/pre-step6`.
