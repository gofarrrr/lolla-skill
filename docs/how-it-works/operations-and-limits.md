...
| `LOLLA_AUDIT_MODE` | No | Metadata-only audit mode. Accepted values: `quick`, `standard`, `deep`, `high_stakes`, `stability`. Missing or empty defaults to `standard`; invalid explicit values fail before model calls. The normalized value is persisted as `risk_mode`, but currently does not change prompts, cost, Step 7, replay, or high-stakes policy. |
...
| `LOLLA_PRE_STEP6_PORTFOLIO_CACHE_REF` | No | Explicit precomputed card-deck file (or cache-dir-relative key/file) to load when the exact compiled-key cache file is absent. Use only for controlled, human-selected cache-hit tests. |
...
|-----------|-------------|
| Host captures the conversation | The transcript is supplied to a private runtime helper on standard input and written owner-only. It is not created through Apply Patch or a file editor, so the user should not see a source-code `Added ...conversation.txt` diff. The host may still show a generic runtime-tool action. |
...
| Conversation is very long | The complete available prose remains authoritative in `conversation.txt`. Above the 80,000-character initial-extraction threshold, `run_extract.py` creates a separate first-3-plus-last-15 message `conversation_processing_view.txt` and exact JSON omission metadata. Run health degrades as `extraction_processing_view_partial`; later conversation-native pressure loads the full source, but that does not prove the partial initial scaffold was semantically adequate. |
| Pipeline finds zero tendencies | Valid outcome. "No structural pressures detected." |
| OpenRouter times out or interrupts extraction | Boundary client returns a non-`ok` status such as `timeout`, `http_error_*`, `url_error`, `response_json_error`, or `provider_finish_error`. Extraction checks this before required semantic fields, preserves all current and earlier attempt records, seals the run, records `extraction_failed`, writes one exact receipt and minimal private failure archive, and stops before the graph. There is no same-run automatic or manual retry. A new attempt requires a new `$lolla` run ID. The only application-level retry remains the single quote-fabrication correction inside a successfully completed extraction invocation. |
...
A typical run makes 18-25 OpenRouter calls against `google/gemini-3.1-flash-lite`:
