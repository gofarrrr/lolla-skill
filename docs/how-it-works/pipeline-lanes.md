...
The model identifier IS persisted to every product run: `audit_summary.boundary_summary.models` is a deduplicated list of every backing model used in the run. Boundary telemetry also records requested and served OpenRouter model IDs so provider substitutions can be flagged instead of silently entering the evidence base. This is the lightweight version of "pin the model and re-baseline" — future feedback can be attributed to specific backing-model routes without paying the cost of pinning.
...
**Calibration approach:** The detection prompt was tuned against 14 test scenarios (in `scripts/test_lane4.py` and `scripts/test_lane4_round2.py`) using the then-production OpenRouter model. Calibration results: 67% recall on expected gaps, ~3 false positives per scenario (capped at 5), consistent across all 4 question types. This calibration level is appropriate for an informative lane where the human filters and the questions carry the value. Known limitations: `feedback-system-dynamics` and `uncertainty-type` are under-detected; `commitment-reversibility` and `stakeholder-alignment` are over-flagged. These can be revisited by tuning detect_when conditions in the knowledge graph.
...
**Graph survival report (archive-time research artifact):** `archive_run.py` renders `graph_survival_report.json` and `graph_survival_report.md` after the Step 6 ledgers are copied. The report joins lane candidates, embedding recalls, selected V60 cards, skipped candidates, V60 transactions, and pre-Step-6 private-table dispositions into one survival view: selected, visibly used, private guardrail, confirming support, rejected, suppressed by packet cap, duplicate, missing source record, or unadjudicated. It deliberately does not call suppressed candidates "noise"; unselected mental models and antagonist approaches remain `unknown_noise_status` until later eval or outcome review shows whether they were useful.

**Pre-Step-6 private table (live skill, context only):** `run_pipeline.py --pre-step6-portfolio step6_private` writes `/tmp/lolla_<run_id>_pre_step6_private_table.md` and serializes `pre_step6_private_table`. The table is a compact private rendering of the current four lanes plus V60, with cached portfolio cards appended only on cache hit. It adds zero LLM calls, never builds live decks, never calls reviewers, and never lets code choose the visible answer. Step 6 reads the table as private context and later records `pre_step6_private_table_ledger`.

**Pre-Step-6 shadow portfolio (explicit research/observability mode):** `run_pipeline.py` still supports `--pre-step6-portfolio shadow`, configurable through `LOLLA_PRE_STEP6_PORTFOLIO=shadow` and `LOLLA_PRE_STEP6_PORTFOLIO_CACHE_DIR`. It records cached-card policy evidence only and leaves visible Step 6 unchanged. Shadow mode computes a compiled card-deck key from the current result payload, checks for a cached deck, derives a ledger signal when a Step 6 ledger is supplied, applies deterministic guardrails, and records `pre_step6_shadow_portfolio` plus a sidecar for archive/Observatory. The legacy shadow gates intentionally stay closed.
...
- `issues` — legacy array naming what happened: `substrate_empty`, `embeddings_off`, `no_fingerprint`, `pipeline_warnings`, `vendor_boundary_reasoning_leak`, `capture_degraded`, `capture_critical`, `quote_fabrication`, `extraction_processing_view_partial`, `lane3_all_dropped`, `bullshit_index_partial`, `stakeholder_check_failed`, `v60_enrichment_failed`, `v60_consideration_ledger_missing`, `v60_consideration_ledger_invalid`, `product_output_leak`, `live_output_leak`, `live_output_missing`, `live_output_unverified`. `capture_truncated` remains a boolean compatibility field for old consumers, not the new issue code and not a claim that the authoritative source was cut.
