...
- twelve fixtures cover `quick`, `standard`, `deep`, `high_stakes`,
...
Status: completed as
[risk-mode-fixture-review-v0.md](../evals/risk-mode-fixture-review-v0.md).

Goal: review the PR37 fixture matrix before runtime enforcement, caller-action
changes, domain protocols, or judges.

Outcome:

- all 11 original PR37 fixtures passed review;
- one missing high-stakes user-values/stakeholder-obligation conflict fixture
  was added and reviewed;
- 12 total fixtures now pass;
...
Goal: make high-stakes reliance caveats explicit in deterministic
`evaluation.json` checks while keeping PR40's caller-action contract unchanged.

Outcome:

- added `risk_mode_reliance_policy` for `risk_mode: high_stakes`;
- otherwise clean high-stakes runs surface the reliance caveat while preserving
  `caller_action: ask_user_first` and `caller_readiness: inspect_first`;
- degraded high-stakes runs surface that risk mode does not override degraded
  run state, preserving `do_not_use_run_degraded` / `do_not_use`;
- standard clean and standard degraded runs do not receive a high-stakes
  caveat;
- no prompt, `SKILL.md`, model-call, archive, caller-action, answer-quality
  scoring, domain-approval, or judge behavior changed.
...
Implemented next as PR64:

- [accountability-view-fixture-review-v0.md](../evals/accountability-view-fixture-review-v0.md)
- [review.json](../../reviews/human/accountability-view-fixture-review-v0/review.json)

PR64 reviewed all three bundles, marked all three pass, and recommended only
`audit_decision_record` for a later exporter-design decision. Provenance and
...
Implemented next as PR66:

- [audit-decision-record-readonly-exporter-v0.md](../evals/audit-decision-record-readonly-exporter-v0.md)
...
PR66 implements only a read-only `lolla.audit_decision_record.v0` exporter. It
reads structured/custody-safe JSON surfaces, writes only an explicit external
output file, refuses output inside the run directory, and keeps raw transcript,
memo, revised-answer, provider/model text, private reasoning, model calls,
