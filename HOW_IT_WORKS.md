...
| `live_constraints` | Every constraint the user stated. Each item carries a `canonical_key` (stable 2–4 token slug identifying the subject across runs), a terse `constraint` string (≤120 chars, noun-phrase + state), plus `status: active / dropped / modified` and `weight: structural / situational`. | **The killer feature of conversation mode.** A constraint stated in turn 3 but absent from the recommendation in turn 8 is omission evidence. Lane 1 triage uses this to detect doubt-avoidance, availability-misweighing, etc. `canonical_key` decouples concept identity from phrasing so cross-run drift measurement tracks "same concept" instead of "same words"; `run_extract.py` blanks any slug that fails the format rule, and the invalid rate is surfaced via `capture_warnings`. |
