...
The [role/attribution case candidate](docs/conversation-understanding/lolla-consumer-context-role-attribution-case-candidate-result-2026-07-23.md) freezes one retrospective safe case, five provider-neutral envelopes, byte-equality and payload-bijection receipts, and an honest blocked F1 slot. The human lane remains blank and paused; the completed [agent-only graph-increment rehearsal](docs/conversation-understanding/lolla-agent-only-graph-increment-rehearsal-result-2026-07-23.md) preserves six first-terminal contexts and a blind non-scalar fan-in. Both reviewers found a material pair difference without identifying lineage. The completed [variance calibration](docs/conversation-understanding/lolla-agent-only-graph-variance-calibration-result-2026-07-23.md) then attempted two exact reruns per condition. Three generations completed, one direct draw failed without replacement, and both blind reviews completed. Both reviewers found a material difference inside the available within-graph pair; the missing draw made the within-direct and one cross pair unavailable. The frozen question is `not_evaluable`, so no graph attribution or expansion follows. The completed [single-case replication](docs/conversation-understanding/lolla-agent-only-graph-replication-result-2026-07-23.md) repaired generation missingness: all eight new direct/graph outputs completed and all eight pairs were available. One blind review was valid; the second used arrays for 29 scalar enum fields and failed the frozen response shape. It was preserved without repair or retry, the post-reveal gate stayed closed, and the result was `not_evaluable`. The later [review-envelope v2 run](docs/conversation-understanding/lolla-agent-only-graph-review-envelope-v2-result-2026-07-24.md) fixed that blind-review shape: both fresh blind reviews passed and opened the gate. Both conditional post-reveal payloads then paraphrased rather than exactly preserved the frozen nonclaim list, failed one deterministic check each, and were preserved without repair or semantic salvage. The result remains `not_evaluable`. The provider-free [V3 nonclaim-custody repair](docs/conversation-understanding/lolla-agent-only-graph-review-nonclaim-custody-v3-repair-result-2026-07-24.md) removes that copying task prospectively: exact nonclaims belong to stable-ID input custody and the model response contains no echo. Its fixtures pass, but no semantic context ran. These are evaluator-envelope findings, not graph causation, quality, expected behavior, or usefulness. This remains Product Delta development evidence—not completed F2/F3, self-justification, graph causation or value, expected model behavior, or human usefulness. V0 is the preserved predecessor.
