...
The [live-run interface and passage truthfulness repair](docs/conversation-understanding/lolla-live-run-interface-and-passage-truthfulness-repair-result-2026-07-24.md) is implemented and provider-free validated; repository publication is pending. It corrects the remaining terminal-startup race through `PRIVATE_INPUT_READY`, gives the optional passage checker complete available user-turn context with exact custody, removes globally repeated dropped-thread hints from each passage call, and aligns partial receipts with review-only machine handoff. It changes no graph behavior and makes no usefulness claim.
