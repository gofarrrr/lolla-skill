...
```text
PR67 Audit Decision Record Export Smoke / Review v0 is the
...
> "not supplied / not inferred" non-claims.
...
- lands `docs/evals/audit-decision-record-export-smoke-review-v0.md`;
- lands `reviews/human/audit-decision-record-export-smoke-review-v0/review.json`;
- reviews six `lolla.audit_decision_record.v0` smoke exports: four from
  existing reviewed archives and two from fixture-backed temp runs;
- confirms records are understandable, artifact statuses are useful, custody
  flags and limitations are clear, raw/private/path safety holds, and no answer
  scoring or approval is implied;
- finds empty PR31 buckets are safe but only partly clear in real archive
  exports because they may look like "no meaningful delta" rather than "labels
  were not supplied or inferred";
- recommends PR68 Audit Decision Record Schema/Exporter Refinement v0 before
...
PR67 only reviews PR66 exporter outputs. Do not start PR68 automatically. Do
not implement runtime integration, archive mutation, automatic generation,
batch export, provenance export, conflict-register export, case-graph export,
graph DB, embeddings, memory, GraphRAG, entity resolution, conflict resolution,
severity automation, policy enforcement, labels, scoring, judging, or any other
accountability primitive from PR67 alone.
```
...
- [ ] Semantica-inspired accountability primitives beyond the first safe
  ...
  and recommends a small schema/exporter refinement. No archive integration,
...
2. **User-values/priorities worksheet v0 is paused after review.** PR34 designs the signal,
   ...
   memory, or judge.
3. **PR68 Audit Decision Record Schema/Exporter Refinement v0, only after
   maintainer review.** PR67 recommends a small refinement so empty PR31 buckets
   and semantic population policy are harder to misread. Any PR68 must remain
   explicitly approved and must not add archive integration, automatic
   generation, labels, answer-quality scores, `safe_for_agent_use`, domain
   approval, runtime integration, graph DB, memory, GraphRAG, judges, automatic
   labels, or Semantica-style platform work.
