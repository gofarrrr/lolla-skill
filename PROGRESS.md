...
The Decision Work Runtime Sidecar Write Contract is
docs/conversation-understanding/decision-work-runtime-sidecar-write-contract-v0.md.
It adds PR209's contract/docs/schema/tests-only gate for a future explicit
operator sidecar write adapter. The contract defines accepted sidecar update
packet and dry-run result inputs, required preconditions, disabled/dry-run/
explicit-operator/future-runtime modes, write statuses, allowed files, path
...
The Decision Work Controlled Archive Sidecar Write Fixture Plan is
...
It defines the next boundary after explicit operator target writes: synthetic
archive-shaped fixture directories. It selects
...
The Decision Work Real Archive Sidecar Write Plan is
...
It defines the first controlled real archive sidecar write boundary:
explicit operator confirmation, completed-run archive markers, matching
sidecar update packet and dry-run result, no-overwrite behavior, receipt
semantics, launch/deploy behavior, and fail-closed refusal rules. It selects
proceed_to_real_archive_sidecar_write_adapter for PR219 while still not
