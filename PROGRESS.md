...
```text
PR40 Risk Mode Contract Lock Tests v0 is the latest risk-mode slice recorded in
...
> fixtures, fixture review, a pre-code implementation plan, and contract-lock
> tests.
...
- Risk-mode implementation is now test-locked at the contract level. The next
  step should clarify evaluation artifacts, not enforce runtime behavior.
...
- decide whether `evaluation.json` should make high-stakes reliance caveats and
  degraded reliance states clearer;
- keep PR40 contract-lock tests green;
...
- [ ] Behavioral risk-mode enforcement; PR36 is design-only, PR37 is fixture
  ...
  PR40 is a contract-lock test slice. Current pipeline behavior is still mostly
  metadata-first.
...
1. **Risk-mode evaluation artifact clarity.** Decide whether `evaluation.json`
   should expose high-stakes reliance caveats more explicitly while keeping
   PR40 contract-lock tests green.
...
| R2: Risk Modes | Metadata plus policy/fixture/plan/tests done | `LOLLA_AUDIT_MODE` accepts `quick`, `standard`, `deep`, `high_stakes`, and `stability`; normalized value persists as `risk_mode`; invalid explicit values fail before model calls. The pipeline remains mostly metadata-first. The agent-result contract already keeps otherwise clean `high_stakes` runs conservative with `caller_action: ask_user_first`. PR36 documents policy, PR37 adds fixtures, PR38 reviews them, PR39 plans high-stakes reliance/readiness tightening, and PR40 locks the current conservative contract in tests without implementing enforcement. |
