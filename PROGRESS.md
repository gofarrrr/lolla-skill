...
```text
PR41 Risk Mode Evaluation Artifact Clarity v0 is the latest risk-mode slice recorded in
...
> and deterministic evaluation-artifact clarity for high-stakes reliance.
...
- Risk-mode implementation is test-locked at the contract level, and
  `evaluation.json` now surfaces high-stakes reliance caveats explicitly. The
  next step should expose that caveat in review/corpus surfaces, not enforce
  runtime behavior.
...
- decide how review/corpus surfaces should expose PR41's
  `risk_mode_reliance_policy` caveat;
- keep PR40 contract-lock and PR41 evaluation-artifact tests green;
...
1. **Risk-mode review/corpus surface integration.** Expose PR41's
   `risk_mode_reliance_policy` caveat to reviewers and corpus rows while
   keeping `safe_for_agent_use` human-owned.
