...
## Fresh-Session Handoff: 2026-06-27

Current `origin/main` / `HEAD`:

```text
43e15841a88b114c3186dc6b55f1f9bc322d7863
Ignore signature-only provider reasoning metadata
```
...
> review seed over those six complex runs.
...
- distinguish action-changing friction from smoother prose;
- turn the six human-reviewed complex runs into reusable rubric examples;
- prepare adversarial pair fixtures without introducing a generic LLM judge;
...
- [ ] First-class user-values/priorities extraction.
...
- [ ] `conversation_understanding_ir.v0` or persisted conversation-understanding
  projection.
- [ ] Live-output hygiene checking beyond conservative `not_checked`.
...
1. **Actionable-delta rubric.** PR30 turned the six complex runs into the first
   human-reviewed reference set. PR31 should define what counts as real
   improvement before inventing judges.
2. **Adversarial eval pairs.** Build pairs where the original answer is smoother
   but the revised answer is more decision-protective.
3. **Additional review batch.** Move beyond the six-run seed toward a small
   human-reviewed archive/corpus batch.
4. **Actionable-delta examples.** Define what counts as a real change: action,
   threshold, sequence, evidence gate, stop rule, or user question.
5. **User-values/priorities extraction.** The corpus repeatedly shows this is
   not measured. Design it explicitly instead of hiding it inside another
   specialist.
6. **Span-grounded semantic enrichment.** Existing specialists help with live
   ...
   are settled.
7. **Live-output hygiene.** Decide whether to add a bounded live-surface check
   or keep `not_checked` as an explicit evaluation warning.
8. **Risk-mode behavior.** Once evaluation is sharper, decide how `deep` and
   `high_stakes` modes should change capture, cost, warnings, and review gates.
...
| R6: Evaluation Methodology And Failure Taxonomy | Human-review v0 done; PR30 seed done | `docs/lolla-evaluation-methodology.md`, `docs/evals/lolla-human-review-v0.json`, `docs/evals/lolla-failure-taxonomy.md`, and `docs/evals/human-review-workflow.md` exist. PR14 added the human-owned label contract. PR15 added a synthetic-review boundary so subagents can help without becoming ground truth. PR16 added a validator and prompt so synthetic candidate outputs must match the human-review schema without becoming human labels. PR30 added the first human/product review seed over the six complex baseline runs. |
...
| R8: Calibrated Subjective Judges | Not started | Correctly deferred. Generic LLM judges may punish useful friction. PR30 supplies a six-run human-reviewed seed; the next step is PR31 Actionable Delta Rubric v0, not judge automation. |
