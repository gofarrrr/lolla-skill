"""Stance-object extension for role-explicit chronological position shards."""
from __future__ import annotations

from collections.abc import Mapping
from copy import deepcopy
from typing import Any

from .reasoning_process_chronological_shard_reader_v2 import (
    build_shard_prompts_v2,
    compile_shard_response_recordwise_v2,
    shard_response_schema_v2,
    validate_shard_record_v2,
)
from .reasoning_process_view_specific import ViewSpecificInterfaceError
from .reasoning_process_views import canonical_json_bytes, sha256_bytes

RESPONSE_SCHEMA_VERSION_V4 = "lolla.reasoning_process_chronological_shard_response.v4"
OBSERVATION_SCHEMA_VERSION_V4 = "lolla.reasoning_process_chronological_shard_observation.v4"

STANCE_OBJECT_KINDS = (
    "belief_or_assessment",
    "action_or_proposal",
    "intended_outcome_or_policy",
    "acceptance_or_willingness",
    "reported_position_landscape",
    "unclear",
)
STANCE_EXPRESSION_KINDS = (
    "reported_without_endorsement",
    "uncertain_or_undecided",
    "considering",
    "held_assessment",
    "preference_or_desire",
    "leaning",
    "proposal",
    "provisional_intention_or_plan",
    "decision",
    "commitment",
    "willing",
    "unwilling",
    "conditional_willingness",
    "possibility",
    "constraint",
    "counterpressure",
    "unclear",
)
STANCE_FIELDS = {
    "stance_components",
    "stance_object_fidelity_note",
}
COMPONENT_FIELDS = {
    "temporal_role",
    "stance_object_kind",
    "stance_object_interpretation",
    "stance_expression_kind",
    "source_evidence_ids",
}
ROLE_EVIDENCE = {
    "starting": "starting_state_evidence_ids",
    "current": "current_position_evidence_ids",
    "qualification": "qualification_evidence_ids",
}
ROLE_LIMITS = {
    "starting": (0, 3),
    "current": (1, 4),
    "qualification": (1, 3),
}

STANCE_OBJECT_INSTRUCTION = (
    "Identify the object of each stance before classifying how the source positions the speaker "
    "toward that object. Return separate source-linked components for starting, current, and "
    "qualification roles. A confident or strongly worded belief or assessment is not a decision; "
    "decision means a chosen action, course, or outcome. Intending or deciding to make a proposal "
    "is distinct from willingness to accept the proposal's outcome. Split multiple objects even "
    "when they share one evidence alias. A reported team or third-party position does not imply "
    "user endorsement. Preserve uncertainty, conditions, provisionality, and counterpressure in "
    "both prose and categorical expression. Expression categories are descriptions, not scores or "
    "a hierarchy. Explain the decomposition in stance_object_fidelity_note."
)


def _component_schema() -> dict[str, Any]:
    return {
        "type": "object",
        "description": "One source-linked stance toward one explicitly identified object.",
        "properties": {
            "temporal_role": {
                "type": "string",
                "enum": list(ROLE_EVIDENCE),
                "description": "Existing position role whose evidence supports this component.",
            },
            "stance_object_kind": {
                "type": "string",
                "enum": list(STANCE_OBJECT_KINDS),
                "description": "What proposition, action, outcome, willingness, or reported landscape the stance concerns.",
            },
            "stance_object_interpretation": {
                "type": "string",
                "minLength": 1,
                "maxLength": 300,
                "description": "Concise source-faithful statement of the particular object, without strengthening it.",
            },
            "stance_expression_kind": {
                "type": "string",
                "enum": list(STANCE_EXPRESSION_KINDS),
                "description": "How the source positions the speaker toward this object. Decision is a chosen course, never belief intensity.",
            },
            "source_evidence_ids": {
                "type": "array",
                "minItems": 1,
                "maxItems": 3,
                "items": {"type": "string", "pattern": "^e[0-9]{3}$"},
                "description": "Exact aliases from this component's parent role; duplicates are rejected after generation.",
            },
        },
        "required": [
            "temporal_role",
            "stance_object_kind",
            "stance_object_interpretation",
            "stance_expression_kind",
            "source_evidence_ids",
        ],
        "additionalProperties": False,
    }


def shard_response_schema_v4(view_kind: str) -> dict[str, Any]:
    schema = deepcopy(shard_response_schema_v2(view_kind))
    if view_kind != "position_and_decision_trajectory":
        return schema
    record = schema["properties"]["records"]["items"]
    properties = record["properties"]
    properties["stance_components"] = {
        "type": "array",
        "minItems": 2,
        "maxItems": 10,
        "items": _component_schema(),
        "description": "Object-specific components grouped by explicit starting, current, or qualification temporal role.",
    }
    properties["stance_object_fidelity_note"] = {
        "type": "string",
        "minLength": 1,
        "maxLength": 800,
        "description": "Why the decomposition keeps belief, action, outcome, and acceptance objects distinct without force inflation.",
    }
    record["required"].extend(
        [
            "stance_components",
            "stance_object_fidelity_note",
        ]
    )
    schema["description"] += " Position records decompose stance force by explicit source-linked object."
    return schema


def build_shard_prompts_v4(wrapper: Mapping[str, Any]) -> dict[str, str]:
    base = build_shard_prompts_v2(wrapper)
    if wrapper["packet"]["view_kind"] != "position_and_decision_trajectory":
        return base
    user_prompt = base["user_prompt"].replace(
        "\nQuestion: ", "\nStance-object contract: " + STANCE_OBJECT_INSTRUCTION + "\nQuestion: "
    )
    return {
        "system_prompt": base["system_prompt"],
        "user_prompt": user_prompt,
        "system_prompt_sha256": base["system_prompt_sha256"],
        "user_prompt_sha256": sha256_bytes(user_prompt.encode("utf-8")),
    }


def _validate_components(
    *, record: Mapping[str, Any], errors: list[str]
) -> list[dict[str, Any]]:
    values = record.get("stance_components")
    if not isinstance(values, list) or not 2 <= len(values) <= 10:
        errors.append("stance_components has invalid cardinality")
        return []
    validated: list[dict[str, Any]] = []
    digests: list[bytes] = []
    for index, component in enumerate(values, start=1):
        prefix = f"stance_components[{index}]"
        if not isinstance(component, Mapping) or set(component) != COMPONENT_FIELDS:
            errors.append(f"{prefix} fields do not match stance-object contract")
            continue
        temporal_role = component.get("temporal_role")
        object_kind = component.get("stance_object_kind")
        expression_kind = component.get("stance_expression_kind")
        interpretation = component.get("stance_object_interpretation")
        evidence = component.get("source_evidence_ids")
        if temporal_role not in ROLE_EVIDENCE:
            errors.append(f"{prefix} temporal role is invalid")
        if object_kind not in STANCE_OBJECT_KINDS:
            errors.append(f"{prefix} object kind is invalid")
        if expression_kind not in STANCE_EXPRESSION_KINDS:
            errors.append(f"{prefix} expression kind is invalid")
        parent_evidence = record.get(ROLE_EVIDENCE.get(temporal_role, ""))
        allowed = set(parent_evidence) if isinstance(parent_evidence, list) else set()
        if (
            not isinstance(interpretation, str)
            or not interpretation.strip()
            or len(interpretation) > 300
        ):
            errors.append(f"{prefix} interpretation is invalid")
        if (
            not isinstance(evidence, list)
            or not evidence
            or len(evidence) > 3
            or any(not isinstance(item, str) for item in evidence)
            or len(evidence) != len(set(evidence))
            or not set(evidence).issubset(allowed)
        ):
            errors.append(f"{prefix} evidence is not a unique non-empty parent-role subset")
        normalized = {
            "temporal_role": temporal_role,
            "stance_object_kind": object_kind,
            "stance_object_interpretation": interpretation,
            "stance_expression_kind": expression_kind,
            "source_evidence_ids": list(evidence) if isinstance(evidence, list) else [],
        }
        digest = canonical_json_bytes(normalized)
        if digest in digests:
            errors.append(f"{prefix} exactly duplicates an earlier component")
        digests.append(digest)
        validated.append(normalized)
    return validated


def validate_shard_record_v4(
    record: Mapping[str, Any], *, wrapper: Mapping[str, Any]
) -> dict[str, Any]:
    view_kind = str(wrapper["packet"]["view_kind"])
    if view_kind != "position_and_decision_trajectory":
        validated = validate_shard_record_v2(record, wrapper=wrapper)
        return {**validated, "stance_object_contract_version": "not_applicable_v2_unchanged"}
    if not STANCE_FIELDS.issubset(record):
        raise ViewSpecificInterfaceError("position stance-object fields are missing")
    projected = {key: value for key, value in record.items() if key not in STANCE_FIELDS}
    validated = validate_shard_record_v2(projected, wrapper=wrapper)
    errors: list[str] = []
    components = _validate_components(record=record, errors=errors)
    role_counts = {
        role: sum(component.get("temporal_role") == role for component in components)
        for role in ROLE_EVIDENCE
    }
    for role, (minimum, maximum) in ROLE_LIMITS.items():
        if not minimum <= role_counts[role] <= maximum:
            errors.append(f"{role} stance component count is invalid")
    starting_present = bool(record.get("starting_state_evidence_ids")) and bool(
        str(record.get("starting_position_interpretation", "")).strip()
    )
    if starting_present != bool(role_counts["starting"]):
        errors.append("starting components must be empty or present with the starting role")
    note = record.get("stance_object_fidelity_note")
    if not isinstance(note, str) or not note.strip() or len(note) > 800:
        errors.append("stance object fidelity note is invalid")
    if errors:
        raise ViewSpecificInterfaceError("; ".join(errors))
    return {
        **validated,
        "stance_objects": {
            "stance_components": components,
            "stance_object_fidelity_note": note,
        },
        "stance_object_contract_version": "object_specific_stance_v1",
    }


def compile_shard_response_recordwise_v4(
    *,
    response: Mapping[str, Any],
    wrapper: Mapping[str, Any],
    producer_kind: str,
    producer_id: str,
    record_identity: str,
    call_metadata: Mapping[str, str] | None = None,
) -> dict[str, Any]:
    if wrapper["packet"]["view_kind"] != "position_and_decision_trajectory":
        return compile_shard_response_recordwise_v2(
            response=response,
            wrapper=wrapper,
            producer_kind=producer_kind,
            producer_id=producer_id,
            record_identity=record_identity,
            call_metadata=call_metadata,
        )
    if set(response) != {"status", "records", "global_limitations"}:
        raise ViewSpecificInterfaceError("v4 response envelope fields do not match")
    status = response.get("status")
    records = response.get("records")
    if status not in {"supported", "mixed", "unclear", "not_found"}:
        raise ViewSpecificInterfaceError("v4 response status is invalid")
    if not isinstance(records, list) or len(records) > 2:
        raise ViewSpecificInterfaceError("v4 response records are invalid")
    if status == "not_found" and records:
        raise ViewSpecificInterfaceError("v4 not_found response must be empty")
    if status != "not_found" and not records:
        raise ViewSpecificInterfaceError("v4 non-empty status requires records")
    if not isinstance(response.get("global_limitations"), str):
        raise ViewSpecificInterfaceError("v4 global limitations are invalid")
    observations = []
    custody = []
    for index, record in enumerate(records, start=1):
        digest = sha256_bytes(canonical_json_bytes(record))
        try:
            validated = validate_shard_record_v4(record, wrapper=wrapper)
            observation_id = (
                f"rpshardv4-{wrapper['packet']['case_id']}-"
                f"{wrapper['packet']['view_kind']}-{index:02d}-{digest[:10]}"
            )
            observations.append(
                {
                    "schema_version": OBSERVATION_SCHEMA_VERSION_V4,
                    "observation_id": observation_id,
                    "case_id": wrapper["packet"]["case_id"],
                    "shard_id": wrapper["packet"]["shard_id"],
                    "family": wrapper["packet"]["view_kind"],
                    "interpretation": validated["display_interpretation"],
                    "role_interpretations": validated["role_interpretations"],
                    "stance_objects": validated["stance_objects"],
                    "semantic_status": validated["status"],
                    "role_source_span_ids": validated["role_source_span_ids"],
                    "source_span_ids": validated["source_span_ids"],
                    "raw_record": {
                        "record_identity": record_identity,
                        "record_index": index,
                        "record": record,
                    },
                    "raw_record_sha256": "sha256:" + digest,
                    "provenance": {
                        "producer_kind": producer_kind,
                        "producer_id": producer_id,
                        "call_id": (call_metadata or {}).get("call_id", ""),
                        "model": (call_metadata or {}).get("model", ""),
                        "prompt_sha256": (call_metadata or {}).get("prompt_sha256", ""),
                    },
                    "terminal_state": "admitted",
                    "graph_routing_eligible": False,
                }
            )
            custody.append(
                {
                    "record_index": index,
                    "terminal_state": "admitted",
                    "observation_id": observation_id,
                    "raw_record_sha256": "sha256:" + digest,
                }
            )
        except Exception as exc:  # noqa: BLE001
            custody.append(
                {
                    "record_index": index,
                    "terminal_state": "quarantined",
                    "reason": f"{type(exc).__name__}: {exc}",
                    "raw_record_sha256": "sha256:" + digest,
                }
            )
    admitted = sum(item["terminal_state"] == "admitted" for item in custody)
    quarantined = len(custody) - admitted
    return {
        "schema_version": RESPONSE_SCHEMA_VERSION_V4,
        "status": "chronological_shard_v4_record_custody_complete",
        "records": custody,
        "observations": observations,
        "shard_terminal_disposition": (
            "reviewed_empty"
            if not custody and status == "not_found"
            else "partially_compiled"
            if admitted and quarantined
            else "compiled"
            if admitted
            else "quarantined"
        ),
        "boundary": {
            "model_records_changed": False,
            "stance_object_correctness_inferred_by_code": False,
            "stance_expression_correctness_inferred_by_code": False,
            "object_expression_compatibility_gate_added": False,
            "expressions_compared_or_scored_by_code": False,
            "prose_keyword_gate_added": False,
            "component_evidence_restricted_to_parent_role": True,
            "record_level_validation_weakened": False,
            "semantic_merge_performed": False,
            "global_synthesis_performed": False,
            "direct_graph_routing_allowed": False,
        },
    }
