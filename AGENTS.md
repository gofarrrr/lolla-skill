...
- **Live skill:** `SKILL.md`, `docs/skill/STEPS.md`, `docs/skill/CODEX_LIVE_RUN_BOUNDARY.md`, and the newest named live-skill result. For the current Codex transport and custody repair, read `docs/conversation-understanding/lolla-codex-live-run-boundary-repair-result-2026-07-24.md`.
...
- The current Codex live-run boundary repair extends that readiness repair across the host workflow: exact run handles survive fresh shells; private input protects user-authored payloads; deterministic code owns ledger shape; named consumer packets replace ordinary artifact dumps; final receipts omit local paths; and interface/tool/privacy/cost claims name their observed scope. The provider-free 14-message replay and full 5,236-test suite pass. Canonical publication is in progress; graph, prompts, providers, retry policy, and semantics are unchanged.
...
- Product Delta's agent-only paired screen preserves mixed moves, harms, disagreement, nulls, and stand-down; it validates no graph or usefulness.
