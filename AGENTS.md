...
- The current Codex live-run boundary candidate extends that readiness repair across the host workflow: exact run handles survive fresh shells; private input protects user-authored payloads; deterministic code owns ledger shape; named consumer packets replace ordinary artifact dumps; final receipts omit local paths; and interface/tool/privacy/cost claims name their observed scope. The provider-free 14-message replay passes. Full verification and publication remain pending; graph, prompts, providers, retry policy, and semantics are unchanged.
