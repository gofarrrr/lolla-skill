...
- The Codex live-run boundary repair is published through PR #404 at merge `f799152671730d631488841a039b1358182a16db`. Exact run handles survive fresh shells; private input protects user-authored payloads; deterministic code owns ledger shape; named consumer packets replace ordinary artifact dumps; final receipts omit local paths; and interface/tool/privacy/cost claims name their observed scope. The provider-free 14-message replay and full 5,236-test suite pass; graph, prompts, providers, retry policy, and semantics are unchanged.
