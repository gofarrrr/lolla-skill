...
- The current Codex live-run boundary repair extends that readiness repair across the host workflow: exact run handles survive fresh shells; private input protects user-authored payloads; deterministic code owns ledger shape; named consumer packets replace ordinary artifact dumps; final receipts omit local paths; and interface/tool/privacy/cost claims name their observed scope. The provider-free 14-message replay and full 5,236-test suite pass. Publication is tracked by PR #404; graph, prompts, providers, retry policy, and semantics are unchanged.
