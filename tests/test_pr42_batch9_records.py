...
BATCH_DIR = REPO_ROOT / "data" / "model_affordances" / "batch_9"
...
def test_pr42_batch9_records_validate_against_schema_and_sources() -> None:
...
def test_pr42_batch9_records_match_source_manifest() -> None:
...
def test_pr42_batch9_keeps_absence_records_first_class() -> None:
    absence_counts = {
...
def test_pr42_batch9_models_were_graph_only_before_this_batch() -> None:
    ...
    assert _model_ids(affordances_v9).isdisjoint(APPROVED_BATCH_MODEL_IDS)


def test_pr42_compiled_v10_includes_v9_plus_batch9_and_remains_dormant() -> None:
    affordances_v9 = _load_compiled(AFFORDANCES_V9_PATH)
    ...
    assert affordances_v10["artifact"] == "model_affordances_v10"
    assert affordances_v10["status"] == "draft_review_only"
    assert v9_model_ids.issubset(v10_model_ids)
    assert APPROVED_BATCH_MODEL_IDS.issubset(v10_model_ids)
    assert len(v10_model_ids) == len(v9_model_ids) + len(APPROVED_BATCH_MODEL_IDS)

    metadata = affordances_v10["compile_metadata"]
    assert metadata["contributing_record_count"] == 122
    assert metadata["affordance_count"] == 158
    assert metadata["absence_record_count"] == 229
    assert metadata["validation"]["schema_validation_failure_count"] == 0
    assert metadata["validation"]["source_quote_rejection_count"] == 0


def test_pr42_v10_is_not_imported_by_live_runtime_paths() -> None:
    forbidden = ("affordances_v10", "model_affordances_v10")
