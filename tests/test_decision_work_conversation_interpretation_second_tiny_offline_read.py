...
SOURCE_PACKET_PATH = (
    REPO_ROOT
    / "reviews/codex-assisted/decision-work-conversation-interpretation-second-tiny-offline-read-v0/source-packet.json"
)
DOC_PATH = (
    REPO_ROOT
...
EXPECTED_CASE = "deploy-assisted-intake-routing"
EXPECTED_RUN = "20260627T130339Z_4cd3cb"
...
def test_selected_case_is_exactly_deploy_assisted_intake_routing() -> None:
    selected = _read()["selected_case"]

    assert selected["case_id"] == EXPECTED_CASE
    assert selected["run_id"] == EXPECTED_RUN
    assert selected["decision_family"] == "healthcare_operations_or_deployment"
...
def test_comparison_to_first_tiny_read_supports_schema_formalization() -> None:
    comparison = _read()["comparison_to_first_tiny_read"]

    assert _repo_ref_exists(comparison["first_read_ref"])
    assert comparison["first_case_id"] == "launch-public-enterprise-beta"
    assert comparison["second_case_id"] == EXPECTED_CASE
    assert comparison["same_tiny_field_set_worked_on_different_decision_family"] is True
    assert comparison["supports_formalizing_shared_schema"] is True
    assert "likely_starting_direction" in comparison["fields_source_limited_in_both_cases"]
    assert "lost_value" in comparison["fields_source_limited_in_both_cases"]
...
def test_brief_implications_and_overclaim_risk_are_present() -> None:
    ...
    assert implications["do_not_modify_rendered_brief_in_pr132"] is True
...
def test_docs_and_read_pass_product_delta_boundary_lint() -> None:
    report = lint_product_delta_paths(
        [DOC_PATH, READ_PATH, PR130_DOC_PATH, PR131_DOC_PATH, PRD_PATH]
    )
...
def test_checked_in_pr132_files_do_not_include_private_markers() -> None:
    paths = [
        READ_PATH,
        DOC_PATH,
        Path(__file__),
...
def test_json_refs_resolve() -> None:
    ...
    for ref in [
        ...
        _read()["comparison_to_first_tiny_read"]["first_read_ref"],
    ]:
        assert _repo_ref_exists(ref)
