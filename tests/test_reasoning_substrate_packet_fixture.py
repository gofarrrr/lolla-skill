...
def test_pr27_fixture_mixes_v4_graph_only_and_suppressed_candidates() -> None:
    ...
    assert packet["coverage_summary"]["v4_reviewed_card_count"] == 3
    ...
    for model_id in (
        ...
        assert card["source_custody"]["v4_reviewed_record_available"] is True
        assert card["source_custody"]["v4_reviewed_affordance_available"] is True
    ...
    for model_id in (
        ...
        assert card["source_custody"]["custody_status"] == "repo_source_custodied"
        assert card["source_custody"]["v4_reviewed_record_available"] is False
        assert card["source_custody"]["v4_reviewed_affordance_available"] is False
        assert card["source_custody"]["manifest_path"].startswith("data/model_sources/")
        assert "No reviewed v4 affordance record" in card["do_not_overclaim"][0]
