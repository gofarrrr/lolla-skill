...
DOC_PATH = (
    ...
    / "decision-work-brief-second-tiny-case-pilot-v0.md"
)
...
USEFULNESS_REVIEW_DOC_PATH = (
    REPO_ROOT
    / "docs/conversation-understanding/"
    / "decision-work-brief-usefulness-review-v0.md"
)
RENDERER_DOC_PATH = (
    REPO_ROOT
...
PILOT_SCHEMA_VERSION = "lolla.decision_work_brief_second_tiny_case_pilot.v0"
BRIEF_SCHEMA_VERSION = "lolla.decision_work_brief.v0"
FIRST_CASE_ID = "ceo-remove-founding-cofounder"
SECOND_CASE_ID = "launch-public-enterprise-beta"
...
REQUIRED_NON_CLAIMS = {
    ...
    "one_or_two_cases_are_not_general_evidence",
...
def _brief() -> dict[str, Any]:
    return _review()["second_case_brief"]["brief"]
...
def test_review_json_has_expected_schema_and_top_level_fields() -> None:
    ...
    assert review["pilot_metadata"]["generated_by"] == "codex_assisted_pr119_review"


def test_selected_case_is_one_non_cofounder_completed_run() -> None:
    selected = _review()["selected_case"]

    assert selected["case_id"] == SECOND_CASE_ID
    assert selected["run_id"] == "20260627T104146Z_7bfe79"
    assert selected["archive_relpath"] == f"{SECOND_CASE_ID}/20260627T104146Z_7bfe79"
    assert selected["case_id"] != FIRST_CASE_ID
    assert selected["not_reused_case"] == FIRST_CASE_ID
...
def test_pr115_packet_was_generated_locally_as_metadata_only() -> None:
    ...
    assert review["input_scope"]["local_private_text_used_for_pr119"] is False
    assert len(packets) == 1
    packet = packets[0]
    assert packet["packet_schema_version"] == "lolla.decision_work_brief_packets.v0"
    assert packet["packet_mode"] == "metadata_only"
    assert packet["generated_locally_for_pr119"] is True
...
def test_custody_flags_are_conservative() -> None:
    for custody in (
        ...
        _review()["second_case_brief"]["custody_flags"],
    ):
    ...
    assert _review()["custody_flags"]["semantic_interpretation_performed"] is True
    assert _review()["custody_flags"]["codex_assisted"] is True
...
def test_second_case_brief_embeds_pr114_brief_contract() -> None:
    brief = _brief()
    ...
    assert brief["brief_metadata"]["case_id"] == SECOND_CASE_ID
...
def test_uncertainty_action_consequence_and_human_followups_are_visible() -> None:
    ...
    assert (
        ...
        or review["second_case_brief"]["action_consequence_read"]
    )
    assert "possible_overcorrection_or_noise" in sections[
        "what_still_might_be_wrong"
    ]["value"]
    assert review["second_case_brief"]["human_followup_questions"]
...
def test_comparison_to_first_case_and_gate_are_conservative() -> None:
    review = _review()
    comparison = review["comparison_to_first_case"]
    gate = review["decision_gate"]

    assert comparison["first_case_ref"] == FIRST_CASE_ID
    assert comparison["second_case_ref"] == SECOND_CASE_ID
    assert comparison["did_both_cases_produce_concrete_action_consequence"][
        "answer"
    ] == "yes"
    assert comparison["did_second_case_test_different_decision_type"]["answer"] == "yes"
    assert gate["outcome"] in ALLOWED_GATE_OUTCOMES
    assert gate["outcome"] == "proceed_to_small_pattern_review"
...
def test_pr119_artifacts_pass_product_delta_boundary_lint() -> None:
