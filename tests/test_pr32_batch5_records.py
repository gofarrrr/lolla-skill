...
BATCH_DIR = REPO_ROOT / "data" / "model_affordances" / "batch_5"
...
def test_pr32_batch5_records_exist_for_approved_models_only() -> None:
    paths = sorted(BATCH_DIR.glob("*.json"))

    assert {path.stem for path in paths} == APPROVED_BATCH_MODEL_IDS


def test_pr32_batch5_records_validate_against_schema_and_sources() -> None:
...
def test_pr32_batch5_records_match_source_manifest() -> None:
...
def test_pr32_batch5_keeps_absence_records_first_class() -> None:
    absence_counts = {
...
def test_pr32_batch5_models_were_graph_only_before_this_batch() -> None:
    affordances_v5 = _load_compiled(AFFORDANCES_V5_PATH)

    assert _model_ids(affordances_v5).isdisjoint(APPROVED_BATCH_MODEL_IDS)


def test_pr32_compiled_v6_includes_v5_plus_batch5_and_remains_dormant() -> None:
    affordances_v5 = _load_compiled(AFFORDANCES_V5_PATH)
    ...
    assert affordances_v6["artifact"] == "model_affordances_v6"
    assert affordances_v6["status"] == "draft_review_only"
    assert v5_model_ids.issubset(v6_model_ids)
    assert APPROVED_BATCH_MODEL_IDS.issubset(v6_model_ids)
    assert len(v6_model_ids) == len(v5_model_ids) + len(APPROVED_BATCH_MODEL_IDS)

    metadata = affordances_v6["compile_metadata"]
    assert metadata["contributing_record_count"] == 81
    assert metadata["affordance_count"] == 117
    assert metadata["absence_record_count"] == 147
