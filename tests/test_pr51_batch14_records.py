...
BATCH_DIR = REPO_ROOT / "data" / "model_affordances" / "batch_14"
...
EXPECTED_ABSENCE_FIELDS = {
    "agency-without-constraint-map",
    "anticipated-regret-without-probability",
    "attitude-change-without-behavior-evidence",
    "audience-confusion-as-inattention",
    "autonomy-without-competence-relatedness",
    "bias-label-as-proof",
    "bias-list-as-diagnosis",
    "confidence-as-competence",
    "control-as-total-responsibility",
    "discomfort-as-error",
    "effort-as-guarantee",
    "expert-clarity-as-user-clarity",
    "good-reason-as-true-reason",
    "grit-without-stop-condition",
    "known-outcome-as-foreseeable",
    "mindset-as-substitute-for-feedback",
    "motivation-as-reward-size",
    "novice-label-as-dismissal",
    "persistence-as-always-virtue",
    "post-hoc-story-as-accountability",
    "postmortem-without-prior-state",
    "regret-as-decision-rule",
    "restraint-without-environment-design",
    "willpower-as-moral-trait",
...
def test_pr51_batch14_records_exist_for_approved_models_only() -> None:
    paths = sorted(BATCH_DIR.glob("*.json"))

    assert {path.stem for path in paths} == APPROVED_BATCH_MODEL_IDS


def test_pr51_batch14_records_validate_against_schema_and_sources() -> None:
...
def test_pr51_batch14_records_match_source_manifest() -> None:
...
def test_pr51_batch14_source_quotes_are_repo_custodied_exact_substrings() -> None:
    for model_id in sorted(APPROVED_BATCH_MODEL_IDS):
        record = _load_record(model_id)
...
def test_pr51_batch14_records_are_compact_and_absence_first() -> None:
    for model_id in sorted(APPROVED_BATCH_MODEL_IDS):
        record = _load_record(model_id)
...
def test_pr51_batch14_blocks_psychological_theater() -> None:
    absence_fields = {
...
def test_pr51_batch14_models_were_graph_only_before_this_batch() -> None:
    affordances_v14 = _load_compiled(AFFORDANCES_V14_PATH)

    assert _model_ids(affordances_v14).isdisjoint(APPROVED_BATCH_MODEL_IDS)


def test_pr51_compiled_v15_includes_v14_plus_batch14_and_remains_dormant() -> None:
    affordances_v14 = _load_compiled(AFFORDANCES_V14_PATH)
    ...
    assert affordances_v15["artifact"] == "model_affordances_v15"
    assert affordances_v15["status"] == "draft_review_only"
    assert v14_model_ids.issubset(v15_model_ids)
    assert APPROVED_BATCH_MODEL_IDS.issubset(v15_model_ids)
    assert len(v15_model_ids) == len(v14_model_ids) + len(APPROVED_BATCH_MODEL_IDS)

    metadata = affordances_v15["compile_metadata"]
    assert metadata["contributing_record_count"] == 182
    assert metadata["affordance_count"] == 218
    assert metadata["absence_record_count"] == 349
    assert metadata["validation"]["schema_validation_failure_count"] == 0
    assert metadata["validation"]["source_quote_rejection_count"] == 0


def test_pr51_v15_is_not_imported_by_live_runtime_paths() -> None:
    forbidden = ("affordances_v15", "model_affordances_v15")
