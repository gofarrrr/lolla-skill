...
BATCH_DIR = REPO_ROOT / "data" / "model_affordances" / "batch_7"
...
def test_pr36_batch7_records_exist_for_approved_models_only() -> None:
    paths = sorted(BATCH_DIR.glob("*.json"))

    assert {path.stem for path in paths} == APPROVED_BATCH_MODEL_IDS


def test_pr36_batch7_records_validate_against_schema_and_sources() -> None:
...
def test_pr36_batch7_records_match_source_manifest() -> None:
...
def test_pr36_batch7_source_quotes_are_repo_custodied_exact_substrings() -> None:
    for model_id in sorted(APPROVED_BATCH_MODEL_IDS):
        record = _load_record(model_id)
...
def test_pr36_batch7_keeps_absence_records_first_class() -> None:
    absence_counts = {
...
def test_pr36_batch7_models_were_graph_only_before_this_batch() -> None:
    affordances_v7 = _load_compiled(AFFORDANCES_V7_PATH)

    assert _model_ids(affordances_v7).isdisjoint(APPROVED_BATCH_MODEL_IDS)


def test_pr36_compiled_v8_includes_v7_plus_batch7_and_remains_dormant() -> None:
    affordances_v7 = _load_compiled(AFFORDANCES_V7_PATH)
    ...
    assert affordances_v8["artifact"] == "model_affordances_v8"
    assert affordances_v8["status"] == "draft_review_only"
    assert v7_model_ids.issubset(v8_model_ids)
    assert APPROVED_BATCH_MODEL_IDS.issubset(v8_model_ids)
    assert len(v8_model_ids) == len(v7_model_ids) + len(APPROVED_BATCH_MODEL_IDS)

    metadata = affordances_v8["compile_metadata"]
    assert metadata["contributing_record_count"] == 98
    assert metadata["affordance_count"] == 134
    assert metadata["absence_record_count"] == 181
    assert metadata["validation"]["schema_validation_failure_count"] == 0
    assert metadata["validation"]["source_quote_rejection_count"] == 0


def test_pr36_v8_is_not_imported_by_live_runtime_paths() -> None:
    forbidden = ("affordances_v8", "model_affordances_v8")
