...
BATCH_DIR = REPO_ROOT / "data" / "model_affordances" / "batch_4"
...
def test_pr28_batch4_records_exist_for_approved_models_only() -> None:
    paths = sorted(BATCH_DIR.glob("*.json"))
    assert {path.stem for path in paths} == APPROVED_BATCH_MODEL_IDS


def test_pr28_batch4_records_validate_against_schema_and_sources() -> None:
...
def test_pr28_batch4_records_match_source_manifest() -> None:
...
def test_pr28_batch4_source_quotes_are_repo_custodied_exact_substrings() -> None:
    for model_id in sorted(APPROVED_BATCH_MODEL_IDS):
        record = _load_record(model_id)
...
def test_pr28_batch4_keeps_absence_records_first_class() -> None:
    absence_counts = {
...
def test_pr28_batch4_models_were_graph_only_before_this_batch() -> None:
    affordances_v4 = _load_compiled(AFFORDANCES_V4_PATH)

    assert _model_ids(affordances_v4).isdisjoint(APPROVED_BATCH_MODEL_IDS)


def test_pr28_compiled_v5_includes_v4_plus_batch4_and_remains_dormant() -> None:
    affordances_v4 = _load_compiled(AFFORDANCES_V4_PATH)
    affordances_v5 = _load_compiled(AFFORDANCES_V5_PATH)
    v4_model_ids = _model_ids(affordances_v4)
    v5_model_ids = _model_ids(affordances_v5)

    assert affordances_v5["artifact"] == "model_affordances_v5"
    assert affordances_v5["status"] == "draft_review_only"
    assert v4_model_ids.issubset(v5_model_ids)
    assert APPROVED_BATCH_MODEL_IDS.issubset(v5_model_ids)
    assert len(v5_model_ids) == len(v4_model_ids) + len(APPROVED_BATCH_MODEL_IDS)

    metadata = affordances_v5["compile_metadata"]
    assert metadata["contributing_record_count"] == 65
    assert metadata["affordance_count"] > affordances_v4["compile_metadata"][
        "affordance_count"
    ]
    assert metadata["absence_record_count"] > affordances_v4["compile_metadata"][
        "absence_record_count"
    ]
    assert metadata["validation"]["schema_validation_failure_count"] == 0
    assert metadata["validation"]["source_quote_rejection_count"] == 0


def test_pr28_v5_is_not_imported_by_live_runtime_paths() -> None:
    forbidden = ("affordances_v5", "model_affordances_v5")
