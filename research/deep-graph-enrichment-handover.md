...
2. **Phase 3 noise-floor threshold.** Below this cosine similarity, the tiebreaker abstains (falls back to deterministic ordering). Set from Phase 0.5 distribution + fixture data — so that the threshold naturally filters out matches against the ~35% situational activation_conditions from Layer 1+2.
   ...
   **Commit B calibration protocol (REV-7, 2026-04-21):** the order below is deliberate — each step must land green before the next starts.
   ...
   4. **Author ~30 fixtures under 14e blind protocol** — 10 near-tie (expect tiebreaker to fire + pick correctly), 10 non-tie (expect byte-identical to current default path), 10 noise-floor (expect abstention). Fixtures live in `tests/fixtures/{graphs,reasoning_shapes,expectations}/` OR under a scenario-json structure per the starter set. A same-session author of the matcher should NOT also author fixtures without a second-reader review — the self-fulfilling-vocabulary trap is real.
      - **STARTER SET landed 2026-04-21 (same-session authorship, pending second-reader review):** `tests/fixtures/activation_tiebreaker/` contains 3 scenarios on the `latticework-of-mental-models` near-tie seed (top-1 systems-thinking, top-2 first-principles-thinking, δ=0.00083). Frozen probe vectors ship in `frozen_probes.json` so tests stay offline. The scenarios test **mechanical faithfulness** (probe aligned with curator AC-A → matcher prefers edge A) — NOT "model X is the right antidote for Y" (semantic claim). Test file: `tests/test_activation_tiebreaker_realdata.py`. Covers: non-swap (probe aligned with top-1), swap (probe aligned with top-2), abstention (off-topic baking probe, both cosines below 0.45 floor).
      - **Expansion via option 2 (programmatic synthesis) landed 2026-04-21:** `scripts/synthesize_tiebreaker_fixtures.py` walks the full near-tie pool in the compiled graph, selects every k-th seed by sorted id to spread across the graph, uses each target model's `select_when[0]` (third-party curator prose authored for a different purpose than AC) as the probe, and picks off-topic probes by SHA-256 hash of the seed. Expected outcome for every scenario is DERIVED from cosine math against the live embeddings DB — no taste calls. This satisfies §14e without a second-reader because the author never chooses the outcome; reality classifies each scenario. First run produced 90 scenarios across 30 seeds, distribution `{swap: 6, no_swap: 9, abstain: 75}`. The heavy abstain tail is honest signal — `select_when` prose is broad-applicability register, deliberately conservative against narrow ACs, and the noise-floor guard correctly protects the default path. All 93 subtests (3 starter + 90 synth) green under `pytest tests/test_activation_tiebreaker_realdata.py`.
      - **Remaining fixture work (still doctrine-gated):** blind-authored scenarios that carry semantic claims of the form "model X is the right antidote for reasoning Y." Those claims need a second reader or a fresh session — option 2 does NOT produce them because it deliberately sidesteps semantic assertions in favor of structural ones.

   5. **Wire `reasoning_context` into `RelationGraph.neighborhood()`** — optional kwarg. When present AND top-2 fan-adjusted delta < ε AND top activation-match similarity ≥ noise-floor: reorder the top-2. Otherwise: untouched. **DONE 2026-04-21**: `engine/system_b/relation_graph.py` now accepts `reasoning_context`, `embeddings_db_path`, `openai_api_key` kwargs plus an `_activation_matcher` test seam. Gate logic lives in `_activation_retie_if_near_tie`. Constants `_ACTIVATION_MATCH_EPSILON = 0.01` and `_ACTIVATION_MATCH_NOISE_FLOOR = 0.45` pinned to the measurements in step 2/3, with `test_relation_graph_activation_tiebreaker.py::CalibrationConstantsTests` guarding against drift. 15 invariant tests cover gate activation logic (default path unchanged, ε gate holds, noise-floor holds, swap fires, relevance_scores overrides, graceful degradation, TypeError propagates). These are ENGINEERING invariants — they do not encode "for reasoning X the right model is Y" claims.
      - **Call-site passthrough landed 2026-04-21 (commit `46d1df0`):** `PressureRouter.route`, `routing.route_tendency` / `route_detected_tendencies` / `route_deep_check_results`, and `PressureBundleSelector.select` / `select_from_packet` all accept and forward the three new kwargs. `tests/test_routing_activation_passthrough.py` (5 tests) verifies each layer both forwards kwargs AND omits them by default. `pipeline.py` intentionally not touched — callers must explicitly opt in. Full suite 81/81, 3 subtests.

   6. **Integration gate — DONE 2026-04-21 (GREEN).** Two orthogonal proofs, both run against the real compiled graph (`data/relationship_graph.json`) and the live backfilled embeddings DB (`data/embeddings.db`, 867 vectors):
      - **(A) Negative / determinism proof:** exercised all 23 subpattern-primary tendencies through `PressureRouter.route()` twice. Result: default path deterministic across runs; passing `reasoning_context` but omitting `embeddings_db_path` produces byte-identical output to default; 0/23 routes changed when context + DB were supplied. The last number is expected — PressureRouter seeds on a single `primary_model_id`, and the 18% near-tie rate from 14h item 2 step 2 was measured across 204 qualifying seeds in the whole graph, not the narrow 23-tendency subset.
      - **(B) Positive / pulse proof:** on the known near-tie seed `latticework-of-mental-models` (top-2 δ=0.00083, well inside ε=0.01), three probes confirm all three gate arms work against the live OpenAI embedder:
        - systems-aligned probe → no swap (top-1 systems-thinking stays; matcher correctly prefers existing top-1)
