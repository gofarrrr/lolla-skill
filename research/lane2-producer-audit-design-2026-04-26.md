...
2. **Gold labels are span-level, not case-level.** "Marcus should have Endowment Effect, Optionality, Inversion" is too mushy and invites hindsight. The unit is *one concrete assistant reasoning span, with one expected reasoning shape and 1–3 plausible mental models.*
3. **Span labels are drafted source-first, before opening current Lane 2 outputs.** The first labeling pass reads the assistant's answer and marks reasoning spans without consulting Lane 2 anchors. Only after the source-first pass exists do we open the current artifacts and attribute hits/misses/false positives. Otherwise the audit becomes "did Lane 2 find things similar to what Lane 2 found" — circular and useless.
4. **Human review (Marcin) owns ambiguous spans and all `expected_primary_models` labels.** Claude drafts. Marcin owns. Especially: any span where multiple primary models are plausible, and any expected model not already surfaced by current Lane 2.
5. **The audit's product is a failure-owner map, not an architecture proposal.** The output is a per-span attribution table that says "this expected model was lost at *this* stage." The decision tree in §9 maps that table to next-track architecture work, but the memo and the audit script do not propose architecture themselves.

## 1. Purpose

Determine whether Lane 2 producer quality is limited by **fingerprint, recall, verifier, surfacing, or Step 6 consumption.**

Concretely: for every expected mental model on every reasoning span in the audit corpus, we want to attribute presence/absence to one of those five stages, plus a `gold_disagreement` bucket for cases where the expected label itself was wrong.
...
This audit instruments every arrow. For every expected mental model on every reasoning span, we will record whether it survived each stage.
...
|---|---|---|---|
| `third-year-phd-student` | 5 (Optionality, Premortem, Status Quo Bias, Base Rates, Problem Framing And Reframing) | **mixed-positive** (semantically plausible, but stability 0.39 is not strong) | Worth labeling spans to test whether plausibility holds at the span level. |
| `user-launch-independent-fintech` | 2 (Margin Of Safety, Optimism Bias And Planning Fallacy) | cleaner positive control | Low anchor count, concrete runway/launch-plan logic, easy to confirm from spans. |
...
A **reasoning span** is a contiguous substring of the assistant's response that performs one cognitive move (e.g., "weighing $1.3M against $6M" is opportunity-sizing; "if Marcus walks, what is the business worth?" is counterfactual-valuation).

Each span gets:

- `assistant_quote` (exact substring from the assistant's answer)
- `reasoning_shape` (one of the 11 shapes in §6.2)
- `expected_primary_models` (1 model if there is a clear one; max 2 only if the span genuinely contains two distinct reasoning moves)
...
> **Read the assistant answer and mark reasoning spans without looking at Lane 2 anchors for that run.**

For cases where Claude already remembers the anchor set from prior work, the discipline still applies: cite the span first, then think about which model fits, *then* open the current Lane 2 output.
...
Each span gets one broad cognitive shape:
...
- `fingerprint_failed` — the span was not extracted, or was extracted in language too generic to support recall
...
For each *observed* anchor (current Lane 2 output) that does not match a gold span:
...
Any span where the draft gold set has more than one plausible primary, OR where a current accepted anchor feels "not wrong but not load-bearing," gets `ambiguous=yes` and goes to Marcin for review. Marcin's call is final.
...
| `span_id` | human-created span id |
| `assistant_quote` | exact assistant substring |
| `reasoning_shape` | one of the 11 shapes |
| `expected_primary_models` | model display_names expected as primary (verbatim) |
| `acceptable_secondary_models` | plausible but not required |
| `should_reject_models` | tempting false-positive candidates |
| `fingerprint_found_span` | yes / no / partial |
...
| `step6_treatment` | primary / secondary / set_aside / hidden |
...
For every current Lane 2 anchor that does NOT map cleanly to an expected_primary_model on a gold span:
...
| `best_matching_span` | gold span_id or none |
...
- `span_recall` — share of gold spans that fingerprint extracted (or near-equivalent)
- `span_specificity` — share of fingerprint-extracted spans that are *specific enough* to support model recall (judged by reviewer; "weighing options" = not specific; "balancing $1.3M cost against $6M of value" = specific)
...
For each expected model on each gold span, across the 3 runs:
...
- If `span_recall` is weak (< threshold), **decompose fingerprint first.**
- If span recall is good but `candidate_recall@60` is weak, **fix recall** (shape-scoped recall, embedding recall, or substrate change).
...
- `span_recall` ≥ X% — fingerprint extracts the moves we care about
...
2. **Audit artifact** — `scripts/lane2_producer_audit.py` (or a structured table doc, depending on what's faster). Output: per-case `gold_span_rows.csv` and `observed_anchor_rows.csv`, plus an aggregate `metrics.md`.
...
1. **Threshold values for §11.** What numbers count as "acceptable" for span_recall, candidate_recall@60, verifier_acceptance_rate, surfacing_recall@5, noisy_anchor_rate?
