...
> **§4 erratum (post-audit, 2026-04-27):** During step 2 attribution on `mid-level-consultant-report`, reading the conversation file revealed it is **byte-identical** to `mid-level-consultant-decides` (case 3). The "same kickoff family, different conversation" assumption was wrong — they are the same conversation archived as two separate runs. Case 7's role therefore shifts from "false-positive risk control" to "same-source stability test" (case 3 vs case 7 producing different anchor sets on identical source). This is more informative than the original framing — the audit's only true run-to-run stability comparison — but it requires the corrected scope description. Actual corpus: **6 distinct conversations + 1 same-conversation rerun**, not 7 independent cases. See `research/stability-runs/lane2-producer-audit-2026-04-26/synthesis.md` §1 for the consolidated implication.
